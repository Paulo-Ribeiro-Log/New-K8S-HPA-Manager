...
`internal/web/handlers/nodepools_snat.go` + `SNATPortWidget.tsx` — diagnóstico de capacidade SNAT/NAT multi-cloud (AKS, EKS, GKE). Renderizado em `Index.tsx` (`renderTabContent` case `"nodepools"`) acima do `SplitView`, visível quando um cluster está selecionado.
...
**Endpoints** (agnósticos de cloud — mesma URL para AKS/EKS/GKE):
- `GET /api/v1/nodepools/snat?cluster=<context>` — perfil atual; detecta provider pelo prefixo do context e chama CLI correspondente
- `GET /api/v1/nodepools/snat/projection?cluster=<context>` — histórico 30 dias + regressão linear
- `GET /api/v1/nodepools/snat/nodes?cluster=<context>` — breakdown por nó via Prometheus conntrack (funciona para todos os providers)

**Detecção de provider** (`detectSNATProvider`): prefixo do context name → `gke_` = GKE, `arn:aws:eks:` = EKS, demais = AKS.

**Modelos por provider**:

| Provider | Fonte de dados | Fórmula | Portas/IP |
|----------|---------------|---------|-----------|
| **AKS** | `az aks show` → `allocatedOutboundPorts` + `managedOutboundIPs.count` | `usedPorts = nodes × allocatedPorts` / `totalAvailable = IPs × 64000` | 64.000 |
| **GKE** | `gcloud compute routers nats list` → `minPortsPerVm` + count de `natIps` (Cloud NAT) | Idêntica ao AKS | 64.512 |
| **EKS** | `aws ec2 describe-nat-gateways` → count de EIPs na VPC (`NatGatewayAddresses`) | `AllocatedOutboundPorts=0` — modelo de 55k conexões/EIP/destino, não limita por nó | 55.000 |

Para EKS: se `VpcID` já estiver em `eks-clusters-config.json`, evita chamada adicional `aws eks describe-cluster`. O campo `Error` no `SNATProfile` explica o modelo quando `AllocatedOutboundPorts=0`.

**`SNATProfile`** tem campo `CloudProvider string` (`"aks"` / `"eks"` / `"gke"`) — usado pelo frontend para adaptar labels e lógica.

**`nodePoolLabel(labels)`**: detecta o label correto por provider — `kubernetes.azure.com/agentpool` (AKS), `eks.amazonaws.com/nodegroup` (EKS), `cloud.google.com/gke-nodepool` (GKE).

**Histórico SQLite** (`internal/storage/snat_history_store.go`): store `snat_history.db` em WAL mode. Deduplica: no máximo 1 snapshot/hora/cluster. Retenção 90 dias. Métodos: `Save`, `GetRecent(cluster, days)`, `GetLatest(cluster)`, `ComputeSNATProjection`. Para EKS (`AllocatedOutboundPorts=0`) o snapshot **não é salvo** (sem dado de portas para projetar).

**Projeção de crescimento** (`ComputeSNATProjection`): regressão linear sobre `total_node_count`. Confiança: `high` (≥14 pts + ≥7 dias), `medium` (≥5 pts + ≥2 dias), `low`, `none`. Retorna `GrowthPerDay`, `DaysUntilLimit` (-1 = indeterminado), `EstimatedDate`.

**Frontend** — 5 abas manuais (div + estado — nunca shadcn `<Tabs>`), conteúdo adaptado por `cloud_provider`:

- **Diagnóstico**: AKS/GKE → barra de uso, 6 cards de métricas, seção Capacidade, breakdown por pool. EKS → painel informativo de NAT Gateway (EIPs, limite 55k, node groups sem coluna de portas)
- **Financeiro**: AKS/GKE → custo por IP (`IP_PRICE_BRL = 20`), planejamento de capacidade. EKS → explicação custo por GB ($0,045/GB NAT GW)
- **Fórmula**: AKS/GKE → equações ports-per-node com nomes específicos (`allocatedOutboundPorts`/`minPortsPerVm`). EKS → modelo 55k conexões/EIP/destino
- **Projeção**: agnóstica de cloud — `LineChart` de nós ao longo do tempo + `ReferenceLine` em `max_nodes_allowed`
- **Nós**: tabela com conntrack Prometheus para todos os providers. AKS/GKE: barra usa `snat_usage_pct` (conntrack ÷ allocated_ports). EKS: usa `conntrack_usage_pct` (conntrack ÷ kernel max) como proxy

**Integração com análise preditiva**: `NodePoolPredictionsHandler` lê o snapshot mais recente do SQLite e popula `req.SNATContext` (`SNATContextData`) antes de chamar `analyzer.Analyze()`. O prompt inclui seção `# SNAT DO LOAD BALANCER AKS` com status, capacidade, projeção e comandos corretivos. Categorias `"snat"` no schema JSON de `root_cause` e `recommendations`.
