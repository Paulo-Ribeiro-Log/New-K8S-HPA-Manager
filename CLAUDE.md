...
Botão **"Port Forward"** (ícone `Network`) — dois pontos de entrada pro mesmo `PortForwardModal.tsx`: um genérico (barra de abas, ao lado de "Notas", sem pré-preenchimento) e um contextual (`PodQuickViewModal.tsx`, botão na linha de ações da aba Detalhes, pré-preenchido com cluster/namespace/pod do pod sendo visto). Pedido explícito do usuário ("desenvolva um mecanismo extremamente completo e que seja exibido em um modal"). Ver `PORT-FORWARD-PLAN.md`.
