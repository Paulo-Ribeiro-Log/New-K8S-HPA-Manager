...
**Diagnóstico de bloqueio por Enterprise Managed Users (EMU)** (`CreatePR` em `code_editor.go`): orgs com EMU rejeitam PAT clássico (`ghp_*`) na REST API com `"Unauthorized: As an Enterprise Managed User, you cannot access this content"` — confirmado empiricamente contra um repo real. **Git clone/fetch/push continuam funcionando** com o mesmo token (autenticação HTTP separada da REST API), só a criação de PR via API é bloqueada; não há nada a corrigir no código, é política do enterprise sobre o token. Detectado por substring na mensagem do GitHub (independente do status HTTP, que varia) e reescrito como erro estruturado (`error_type: "emu_pat_blocked"` + `instructions[]` passo a passo para gerar um fine-grained token aprovado pelo admin) em vez do texto cru — mesmo padrão já usado para `saml_authorization_required` em `github_releases.go`. **Cuidado ao alterar o interceptor de 403 em `client.ts`**: ele sobrescrevia todo erro 403 por um texto genérico de RBAC K8s, mascarando tanto este `error_type` quanto o de SAML — agora só aplica o fallback quando `rawError.error_type` está ausente, e anexa `details: rawError` ao `Error` lançado para o chamador acessar `error_type`/`instructions` sem precisar bypassar `apiClient.request()` com `fetch` cru (como `GitHubReleasesTab.tsx` já fazia por esse motivo).
