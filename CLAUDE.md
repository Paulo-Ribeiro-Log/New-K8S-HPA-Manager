...
- [**Plano: Migração da autenticação Dynatrace pro Perfil do Usuário**](DYNATRACE-PROFILE-MIGRATION-PLAN.md) ← ⏳ planejado, nenhuma fase iniciada — move a configuração de Dynatrace (URL/token/tag filter) da aba AI Diagnostics pro menu Perfil (mesmo padrão de GitHub/Nexus/ServiceNow/AWX), trocando a identidade de um `ai_email` digitado manualmente pelo e-mail real do login (RBAC/JWT)
