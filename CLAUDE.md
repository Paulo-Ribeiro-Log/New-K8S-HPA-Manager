...
- [**Plano: Descoberta de Prometheus em Clusters GKE**](GKE-PROMETHEUS-DISCOVERY-PLAN.md) ← work in progress — Fase 1 (override manual `prometheusUrl` em `ClusterConfig`/`EKSClusterConfig`/`GKEClusterConfig`) concluída mas não commitada; Fases 2-5 (descoberta automática de URL do Google Cloud Managed Service for Prometheus + auth OAuth2 Bearer via hook `SetGCPTokenFunc`, quebrando um import cycle real entre `internal/cloudprovider/gcp` e `internal/monitoring/discovery`) ainda não iniciadas
