...
**`NoteEntry.tsx` — item colapsável, colapsado por padrão**: componente compartilhado (`Collapsible`/`CollapsibleTrigger`/`CollapsibleContent` de `@/components/ui/collapsible`, mesmo primitivo Radix já usado em `AIHistoryPanel.tsx` para os Filtros Avançados) usado nos 3 lugares que renderizam uma nota — lista escopada do `NotesModal`, resultados de busca e o `GeneralNotesWidget` abaixo. Colapsado (estado padrão): só a linha com autor/data + badges opcionais + preview de uma linha (`firstLine()`, primeira linha não-vazia truncada em 100 chars). Expandido: markdown completo renderizado + botões Editar/Excluir (quando `isAuthor` e os handlers são passados — resultados de busca e o widget geral não passam handlers, então ficam somente leitura). Existe só esse componente — nenhum dos 3 call sites reimplementa a renderização de nota.

**`GeneralNotesWidget.tsx` — post-it de lembretes visível em qualquer aba**: o botão "Notas" da `TabNavigation` é escopado à `activeTab` — muda de conteúdo a cada troca de aba, então não serve para reminders tipo "ajustar X antes de sexta" que precisam ficar visíveis não importa em qual ferramenta o usuário está. Reaproveita 100% do backend existente sem schema novo: `GENERAL_NOTES_TAB = "__general__"` (`hooks/useNotes.ts`) é um valor de `tab` reservado que nunca colide com uma aba real da UI — `useNotes(cluster, GENERAL_NOTES_TAB)` busca essas notas como qualquer outro escopo. Renderizado uma única vez em `Index.tsx` (fora de qualquer bloco condicional por aba, `fixed bottom-4 left-4 z-40` — canto oposto ao dos toasts do sonner, que usam o canto inferior direito por padrão), portanto sobrevive a troca de `activeTab` sem desmontar.

Colapsado por padrão: só um pill amarelo (`StickyNote` + contagem via `Badge`, mesmo `notes.length` do hook). Expandido (clique no pill): painel flutuante com as notas gerais via `NoteEntry` (também colapsadas por padrão) + botão "Gerenciar lembretes", que abre uma instância separada de `NotesModal` travada em `tab={GENERAL_NOTES_TAB}` — reaproveita 100% do editor/toolbar/preview Markdown já existente em vez de duplicar um composer dentro do widget. `NotesModal` ganhou `tabLabel` (`tab === GENERAL_NOTES_TAB ? "Gerais (todas as abas)" : tab`) só para o título do modal não mostrar o sentinel cru.
