...
- [**Plano: Validação de Cadeia de Certificados + Rollback de Atualizações TLS**](CERT-ROLLBACK-VALIDATION-PLAN.md) ← 🔵 planejado, nenhuma fase iniciada — validação de cadeia via Go nativo (`crypto/x509`/`crypto/tls`, sem depender de binário `openssl` — imagem de produção é `FROM scratch`) e mecanismo de rollback (`~/.k8s-hpa-manager/rollback-certs/<secret>/<data>/`) cobrindo tanto upload manual quanto o fluxo AWX (que não passa pelo `Scanner.UploadCertificate`)
