...
- [**Plano: Dynatrace — Diagnóstico Acionável**](DYNATRACE-DIAGNOSTICS-PLAN.md) ← ✅ Fase 1 (regras unificadas) e Fase 2 (navegação real das sugestões — `ActionPlanCard`/`OneAgentSignalCard`/"Onde atuar" agora clicam e abrem HPA/Deployment pré-selecionado, zero mudança de backend) concluídas — validação no navegador da Fase 2 ainda pendente. Fase 3 (distributed tracing) travada numa decisão do usuário: endpoint clássico dá 404 e Grail/Platform dá 403 nesse tenant, provavelmente exige client OAuth2 dedicado (acesso admin no tenant DT, separado do login pessoal) — usuário vai verificar viabilidade antes de qualquer código
