...
`internal/web/handlers/nodepools_snat.go` + `SNATPortWidget.tsx` — diagnóstico de portas SNAT do Load Balancer AKS. Renderizado em `Index.tsx` (`renderTabContent` case `"nodepools"`) acima do `SplitView`, visível quando um cluster está selecionado.
...
**Endpoints**:
- `GET /api/v1/nodepools/snat?cluster=<cluster>` — perfil atual (az aks show + nós K8s); salva snapshot no histórico SQLite de forma assíncrona
- `GET /api/v1/nodepools/snat/projection?cluster=<cluster>` — histórico 30 dias + regressão linear para projeção de crescimento
- `GET /api/v1/nodepools/snat/nodes?cluster=<cluster>` — breakdown por nó via Prometheus (conntrack como proxy de uso SNAT)

**Cálculo**: obtém `allocatedOutboundPorts` e `outboundIpCount` via `az aks show` (timeout 30s) + conta nós ativos de todos os node pools. Fórmula: `totalAvailablePorts = outboundIpCount × snatPortsPerIP (64000)`, `totalRequiredPorts = allocatedOutboundPorts × totalNodeCount`. Status: `ok` (<80%), `warning` (80-95%), `critical` (≥95%).
...
**Histórico SQLite** (`internal/storage/snat_history_store.go`): store `snat_history.db` em WAL mode. Deduplica automaticamente: no máximo 1 snapshot/hora/cluster. Retenção 90 dias. Métodos principais: `Save`, `GetRecent(cluster, days)`, `GetLatest(cluster)`, `ComputeSNATProjection(records, nodesUntilLimit)`.
...
**Frontend**: header compacto sempre visível (status, % de uso, badge de nós disponíveis). Clique abre `Dialog` (`max-w-2xl`, `max-h-[78vh]`) com **5 abas manuais** (div + estado — nunca shadcn `<Tabs>`):
- **Diagnóstico** — barra de uso, portas livres/nós cabem, 6 cards de métricas (grid 3×2), seção Capacidade, breakdown por node pool
- **Financeiro** — custo atual dos IPs no LB (R$/mês e R$/ano), custo adicional para cobrir déficit, planejamento de capacidade por nó. Constante `IP_PRICE_BRL = 20` (IP público Standard Azure Brasil Sul — ajustar se necessário)
- **Fórmula** — equações explicadas passo a passo, thresholds de status, mensagem de solução quando há déficit
- **Projeção** — taxa de crescimento (nós/semana), badge de confiança, dias/data estimada de esgotamento, `LineChart` com `ReferenceLine` em `max_nodes_allowed`
- **Nós** — tabela de nós com mini progress bars de uso estimado SNAT (conntrack ÷ allocated_ports), clique no nó exibe `AreaChart` de histórico conntrack 6h (reutiliza endpoint `/api/v1/nodepools/conntrack/history`). Disclaimer: conntrack inclui tráfego inbound+local — valor é limite superior
...
**Por que não usar Conntrack aqui**: SNAT é uma limitação do Load Balancer ARM (fora do node), não do kernel conntrack. São métricas complementares — conntrack serve como proxy de estimativa por nó, mas o orçamento SNAT real vem do `az aks show`.
