...
- [**Plano: Teste de Banco de Dados sob Demanda**](DATABASE-TEST-PLAN.md) ← ✅ implementado — aba "Teste de Banco de Dados": PostgreSQL/MySQL-MariaDB/MongoDB/Redis via registry de engines, conectividade+autenticação (manual ou lida de Secret/ConfigMap) e navegação só-leitura opcional; modos `pod` (Ephemeral Container) ou `local` (Docker no host). **Nunca escreve no banco**. Não validado ponta a ponta contra bancos reais nesta rodada
