...
**Modelos Gemini Vertex AI**: aliases de input — `gemini-3.5-flash`, `gemini-3.1-pro`, `gemini-2.5-pro`, `gemini-2.5-flash`, `gemini-2.0-flash` — convertidos automaticamente via `ToVertexModelID()` (`vertexModelMap` em `gemini_provider.go`) para IDs versionados do Vertex AI (ex: `gemini-3.5-flash-preview-0514`, `gemini-2.5-pro-preview-06-05`). Modo Vertex AI não aceita aliases do AI Studio diretamente — sempre passar pelo alias para garantir a conversão.

**`analyzeVertex` — prioridade de roteamento** (ordem de precedência):
1. `agentspaceEngineID != "" && project != ""` → Discovery Engine API (`discoveryengine.googleapis.com`) — para WIF sem permissão `aiplatform.endpoints.predict`
2. `project != ""` → Vertex AI direto (`aiplatform.googleapis.com`) com token WIF/Service Account/ADC
3. `project == ""` + refresh token → AI Studio com OAuth token (quota individual Workspace, sem projeto GCP)
4. Fallback: ADC sem projeto configurado (raro)

**`GeminiAgentspaceEngineID`**: Engine ID do Vertex AI Agentspace (CID da URL `vertexaisearch.cloud.google`). Exemplo: `e27c3217-b2b0-4e85-8002-8b0070735c03`. Quando preenchido junto com `GeminiVertexProject`, redireciona todas as chamadas para `discoveryengine.googleapis.com` via `analyzeAgentspace()` — útil para usuários cujo WIF tem acesso ao Agentspace mas não ao `aiplatform`. Salvo em `user_ai_tokens.gemini_agentspace_engine_id` (via migration).
...
| Vertex AI: modelos diferentes do esperado | Modos apikey e vertex têm listas diferentes. Vertex AI usa aliases (`gemini-3.5-flash`, `gemini-3.1-pro`, `gemini-2.5-pro`) convertidos via `ToVertexModelID()` — não usar IDs versionados do AI Studio diretamente |
| Agentspace: erro 403 ao testar conexão | Token WIF não tem acesso à Discovery Engine API — verificar se o pool WIF tem role `Discovery Engine Viewer` no projeto GCP. Detalhes no body do erro |
| Agentspace: erro 404 ao testar conexão | Engine ID incorreto — confirmar o CID na URL `vertexaisearch.cloud.google/home/cid/<ID>` e que o projeto GCP corresponde ao engine |
| Agentspace: resposta vazia (state=…) | Engine configurado mas sem dados indexados ou preamble inválido — testar diretamente na UI do Agentspace |
