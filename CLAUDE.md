...
- [**Plano: Modo Triagem no Health Check**](HEALTHCHECK-TRIAGE-MODE-PLAN.md) ← 📋 planejamento — nenhuma fase iniciada. Nasceu da conversa sobre integração Zabbix: em vez de varrer o cluster ponto a ponto, resolve primeiro quais namespaces têm problema sinalizado por Dynatrace/Prometheus (Zabbix depois, como 3ª fonte) e só then varre a fundo esse escopo reduzido — mantém a Varredura Completa de hoje como modo alternativo (não substitui, perde detecção de postura/higiene se substituísse). "Grafana" confirmado como não sendo integração nova (mesmos alertas do Prometheus, cliente já existe em `internal/monitoring/alerts`, nunca ligado ao Health Check). Granularidade por namespace na Fase 1 (zero mudança nos `*_checker.go`, que já aceitam `namespaces []string` filtrado) — granularidade por workload fica de fase futura condicional
