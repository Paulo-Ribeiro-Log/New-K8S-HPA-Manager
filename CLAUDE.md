...
**`ConntrackAlertWidget.tsx`**: badge compacto sempre visível na aba Node Pools (mesmo padrão visual do `SNATPortWidget.tsx`, renderizado logo abaixo dele em `Index.tsx` case `"nodepools"`), mostrando o nó mais saturado do pool selecionado. Limiares: "Atenção" ≥65% (mesmo valor de `getCapacityRec`), "Crítico" ≥90% (mesmo limiar do backend em `probeConntrack`). Só aparece com um node pool selecionado (o scan é por pool, não há endpoint agregado por cluster). Clique abre modal simples com a lista de nós ordenada por uso.
