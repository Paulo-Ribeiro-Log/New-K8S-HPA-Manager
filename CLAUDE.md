...
### SNAT Port Widget (Node Pools)

`internal/web/handlers/nodepools_snat.go` + `SNATPortWidget.tsx` — diagnóstico do orçamento de portas SNAT do Load Balancer AKS. Renderizado em `Index.tsx` (`renderTabContent` case `"nodepools"`) acima do `SplitView`, visível quando um cluster está selecionado.
...
**Endpoint**: `GET /api/v1/nodepools/snat?cluster=<cluster>`

**Cálculo**: obtém `allocatedOutboundPorts` e `outboundIpCount` via `az aks show` (timeout 60s) + conta nós ativos de todos os node pools. Fórmula: `totalAvailablePorts = outboundIpCount × snatPortsPerIP (64000)`, `totalRequiredPorts = allocatedOutboundPorts × totalNodeCount`. Status: `ok` (<80%), `warning` (80-95%), `critical` (≥95%).

**`SNATProfile`** contém:
- `MaxNodesAllowed` — máximo de nós suportados com o IP atual antes de falha SNAT
- `NodesUntilLimit` — quantos nós ainda cabem
- `IPsNeededForCurrentNodes` — IPs adicionais necessários para cobrir a carga atual

**Frontend**: widget colapsável com barra de progresso colorida (verde/âmbar/vermelho), lista de node pools com contribuição individual, botão de refresh. Cache de 2min via React Query (`staleTime: 2 * 60 * 1000`).

**Por que não usar Conntrack aqui**: SNAT é uma limitação do Load Balancer ARM (fora do node), não do kernel conntrack. São métricas complementares.
...
| Servidor desliga sozinho após ~40min | Auto-shutdown por inatividade (`useHeartbeat.ts` envia a cada 5min; browsers throttleiam em abas de fundo). Reabrir a aba reinicia o heartbeat. Threshold: 40min sem heartbeat → `os.Exit(0)` |
...
| Gemini não autentica no WSL2 | OAuth app-callback usa porta 8080 (forwarded no WSL2) — deve funcionar. Se não abrir, o popup foi bloqueado: clicar no link "Clique aqui para abrir o login" que aparece no estado "aguardando" |
| WIF SSO: campos não populam ao carregar | `gemini_wif_login_url` estava faltando no tipo de `getAITokens()` em `client.ts` — corrigido. Se ainda não popular, verificar se `ai_email` está no `localStorage` e corresponde ao email salvo no backend |
| WIF SSO: autenticar sem preencher Pool/Provider | Sem o campo WIF Pool/Provider preenchido, usa OAuth2 padrão (accounts.google.com) em vez de WIF corporativo. Preencher o formato `poolID/providerID` ANTES de clicar "Autenticar com Google" |
| WIF SSO: "Testar Conexão" falha após OAuth | Refresh token salvo, mas projeto GCP não configurado — preencher Project ID antes de testar |
| Vertex AI: modelos diferentes do esperado | Modos apikey e vertex têm listas diferentes. Vertex AI usa modelos Agentspace (`gemini-3.5-flash`, `gemini-3.1-pro-001`, `gemini-2.5-pro-preview-05-06`) — não aceita IDs do AI Studio |
| Job criado mas sem namespace correto | Backend sempre sobrescreve namespace pelo parâmetro do request — nunca do YAML. Verificar `CreateJobFromYAML` em `internal/kubernetes/client.go` |
| Commit GitHub falha "Not Found" | PAT sem permissão de escrita no repositório ou SSO não autorizado para a org. Verificar scopes do token (precisa `contents: write`) |
...
| Code Editor: context menu não fecha | O `document.addEventListener("mousedown")` depende do `onMouseDown={e.stopPropagation()}` dentro do menu; se faltar, o clique dentro do menu também fecha |
| Code Editor: "Revelar na tree" não destaca o arquivo | Verificar se a aba lateral está em "files" (`setSidePanel("files")` chamado antes de `setRevealPath`); o nó só recebe `data-reveal-path` se `revealPath === node.path` e o dir pai estiver expandido |
| Code Editor: font size não muda no Monaco | `updateOptions` depende de `editorRef.current` estar montado; se o editor foi recriado (troca de arquivo), o efeito de `[fontSize, wordWrap]` dispara novamente e reaplicará; verificar se `handleEditorMount` está sendo chamado |
| Code Editor: Ctrl+P não abre paleta | Verificar se `relative` está no container `<div className="flex-1 flex flex-col min-h-0 min-w-0 relative">` — o overlay usa `absolute inset-0`; também verificar se `selectedRepo` está definido (o listener global filtra por `selectedRepo`) |
| Code Editor: auto-save salva conteúdo desatualizado | `saveFileRef.current` usa closure da última renderização — o `useEffect(() => { saveFileRef.current = saveFile; })` sem deps garante atualização; nunca remover esse efeito |
| Code Editor: format on save não formata | Verificar se a linguagem está na lista `["go","typescript","javascript","python","json"]` e se o endpoint `POST /repos/:id/fmt` retorna conteúdo formatado; erros de format são silenciosos (arquivo salvo sem formatação) |
| Code Editor: clone não exibe nenhuma mensagem | Nunca ocorre na versão atual — CloneDialog trata 409 (já clonado), 400 (limite/disco), erro de rede e sucesso com mensagens distintas |
| Code Editor: "Repositório já clonado localmente" | 409 do backend — clicar "Abrir repositório existente" para navegar ao repo sem re-clonar |
...
| LSP Go: completions não aparecem | Verificar se `gopls` está instalado (`~/go/bin/gopls` ou PATH). Status: `GET /api/v1/code-editor/repos/:id/lsp/status?lang=go`. Primeira completion pode demorar 3-5s (gopls indexando workspace) |
| LSP Go: "language server 'go' não encontrado" | Instalar gopls: `go install golang.org/x/tools/gopls@latest`. Deve ficar em `~/go/bin/gopls` |
| LSP Go: completions duplicadas | `__monacoGoLSPRegistered` evita registrar providers múltiplas vezes. Se apareceu duplicado, verificar se a flag foi removida acidentalmente de `handleEditorMount` |
| LSP: diagnósticos não aparecem | Polling roda apenas quando aba Go/Python está ativa. Verificar `useEffect([activeTabIdx, activeTab.node.path])` no CodeEditorTab. `__lspApplyDiagnostics` deve estar definido após `handleEditorMount` |
| LSP TS/JS: erros "cannot find module" | Esperado — Monaco TS worker não tem acesso ao `node_modules`. Para suprimir, usar `noSemanticValidation: true` em `javascriptDefaults.setDiagnosticsOptions` |
| SNAT widget não aparece na aba Node Pools | Verificar se o cluster está selecionado. O widget fica em `Index.tsx` case `"nodepools"` — **não** em `NodePoolTab.tsx` (componente órfão, nunca importado) |
| SNAT widget não carrega | `az aks show` retorna erro ou timeout (60s). Verificar VPN e autenticação Azure CLI (`az account show`). O widget exibe `error` no campo `status` da resposta |
| SNAT widget mostra "0 IPs" | Cluster sem LB profile ou configuração SNAT padrão — `allocatedOutboundPorts` e `outboundIpCount` = 0. Checar `az aks show --query networkProfile.loadBalancerProfile` |
| HPAEditor: botões desabilitados mesmo sendo SRE | `ProtectedAction` usa `allowed={permissions.canUpdateHPA}` — RBAC K8s prevalece. Se `canUpdateHPA=false`, o kubeconfig do servidor não tem permissão `update` em `horizontalpodautoscalers` naquele namespace |
...
| Code Editor: painel Source Control vazio | Verificar se `git status --porcelain` retorna output — `modifiedPaths` é derivado do status. Pull recente pode não ter conflitos; sem arquivos modificados o painel fica vazio |
| Code Editor: push rejeitado com "non-fast-forward" | Pull --rebase automático foi implementado: o handler tenta `git pull --rebase` antes de retornar erro. Se o rebase falhar (conflito), o push retorna erro com mensagem de conflito |
