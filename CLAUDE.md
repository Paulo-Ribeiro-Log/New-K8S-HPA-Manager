...
- [**Plano: Descoberta de Prometheus em Clusters GKE**](GKE-PROMETHEUS-DISCOVERY-PLAN.md) ← Fases 1-3 concluídas, Fase 4 parcial — URL do Google Cloud Managed Service for Prometheus (GMP) construída automaticamente a partir do Project ID embutido no context GKE, auth OAuth2 Bearer via hook `discovery.SetGCPTokenFunc` (wireado em `internal/config/kubeconfig.go`, não `cmd/web.go`, pra reaproveitar bootstrap GKE já existente e não fechar o import cycle real entre `cloudprovider/gcp` e `monitoring/discovery`). Aplicado nos clientes de query (`monitoring/client`, `monitoring/prometheus`); `monitoring/alerts` fica de fora por motivo estrutural (GMP não expõe `/api/v1/alerts` — não é um Prometheus server, só backend de métricas via PromQL) e os clientes Prometheus internos do FinOps (4º grupo, descoberto nesta fase) ainda não foram cobertos. Fase 5 (validação contra cluster GKE+GMP real) não pode ser feita neste ambiente
