...
- [**Plano: Teste de Latência sob Demanda**](LATENCY-METRICS-PLAN.md) ← ✅ Fases 1-5 concluídas (teste ativo ponta a ponta + guardrails + contexto histórico DT/Prometheus complementar), Fase 6.1 (protocolo ICMP/HTTP/HTTPS via `nicolaka/netshoot`) e Fase 6.2 (alvos multi-cloud — só AWS populado via `s3.<região>.amazonaws.com`, GCP/Azure deliberadamente vazios, ver plano) concluídas; Fases 6.3-6.4 (persistência, grafo de topologia reaproveitando Cytoscape.js do Service Mesh) e Fase 7 (correlação Health Check) pendentes — ferramenta ativa (botão "Executar Teste", aba `latency-test`/`LatencyTestTab.tsx`): pod efêmero + curl/ping via exec (reaproveita `execCmdInPod` do conntrack) mede latência real na hora, SSE de progresso, badge DT/Prom com P95/P99 histórico ao lado do resultado
