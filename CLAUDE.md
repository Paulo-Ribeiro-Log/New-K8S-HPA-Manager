...
- [**Plano: Modo Triagem no Health Check**](HEALTHCHECK-TRIAGE-MODE-PLAN.md) ← 🟢🟢 Fases 1 (backend), 2 (frontend) e 4 (ignore-lists) implementadas e **validadas ao vivo em 2026-08-20 contra um cluster real**; Fase 3 (Elasticsearch) implementada no mesmo dia mas **não validada** contra um índice real (sessão Azure AD expirou no meio da implementação — ver detalhe ao final deste bullet) (`akspriv-abastecimento-hlg`, via API direta com JWT real — 23/26 clusters acessíveis no ambiente daquela sessão). Nasceu da conversa sobre integração Zabbix: em vez de varrer o cluster ponto a ponto, resolve primeiro quais namespaces têm problema sinalizado por Dynatrace/Prometheus (Zabbix depois, Fase 5) e só then varre a fundo esse escopo reduzido — mantém a Varredura Completa de hoje como modo alternativo (não substitui, perde detecção de postura/higiene se substituísse). "Grafana" confirmado como não sendo integração nova (mesmos alertas do Prometheus, cliente já existe em `internal/monitoring/alerts`, nunca ligado ao Health Check). Granularidade por namespace na Fase 1 (zero mudança nos `*_checker.go` de resultado, que já aceitam `namespaces []string` filtrado) — granularidade por workload fica de fase futura condicional. `HealthCheckRequest.TriageMode` + `TargetSource` (`internal/healthcheck/target_resolver.go`, Dynatrace/Prometheus) resolvem o escopo antes do fallback `getAllNamespaces`. Achado real corrigido durante a implementação: `internal/web/handlers/healthcheck.go` só populava credenciais Dynatrace quando `CheckDynatrace`/`CheckOneAgentSignals` estava marcado — sem `|| TriageMode`, a triagem nunca teria credencial DT sozinha. **Bug real achado só na validação ao vivo (não pego por nenhum teste unitário)**: `triage_summary` sempre voltava `null` no `GET /api/v1/healthcheck/:id` — `internal/healthcheck/storage.go` serializa campos sem coluna própria via `extraResultFields`, e `TriageSummary` nunca tinha sido adicionado ali (mesma classe de bug que `TestSaveAndGetHistory_RoundTripsAllExtraFields` já existia pra pegar, com `NodeResults` como precedente documentado no próprio teste) — corrigido (commit `1d9d25df`), teste estendido. **Validado ao vivo depois da correção**: `triage_summary` veio completo com 10 namespaces reais e alertnames reais do Prometheus; confirmado que `deployment_results`/`hpa_results` ficam de fato confinados ao escopo triado (não é só relatório decorativo); Fase 4 confirmada suprimindo 2 alertas reais, derrubando um namespace do escopo. Frontend (`HealthCheckingTab.tsx` toggle "Modo de Verificação", `HealthReportTab.tsx` seção "Escopo da Triagem" com painel farol) só não foi clicado num navegador ainda — risco residual baixo, sem lógica própria além de espelhar tipos. Backlog registrado em 2026-08-20 (a partir da revisão de um script Python de referência de outra ferramenta interna): Fase 3 — Elasticsearch/ELK como fonte de erro de log, contorna o mesmo bloqueio de Grail/Platform documentado na seção Dynatrace acima; Fase 4 — `TriageIgnoreManager` (`internal/healthcheck/triage_ignore.go`, persistência própria, lista de entradas tipadas por `Source` em vez de 4 slices paralelas como o desenho original sugeria) filtra `DynatraceTargetSource`/`PrometheusAlertsTargetSource` antes de popular `Namespaces`/`Reasons`; UI nova aba "Sinal Externo (Triagem)" dentro do `FiltersManagementModal.tsx` existente (abas manuais, nunca shadcn `<Tabs>`). **Fase 3 implementada em 2026-08-20**: `internal/elasticsearch/` (pacote novo, client HTTP mínimo Basic Auth) + `ElasticsearchTargetSource` — convenções de campo assumidas por decisão explícita do usuário ("assumir e ajustar depois", não bloquear o código até confirmar manualmente): `@timestamp`, `level` (casa com o formato de log já documentado nesta app pro pipeline FluentD+EventHub), `kubernetes.namespace_name`. Achado de design não coberto pelo desenho original: a query precisa filtrar por `cluster_name` (suposição adicional) — esta app gerencia ~26 clusters, um índice/pipeline de log compartilhado é o caso comum, sem esse filtro logs de clusters diferentes se misturariam na mesma contagem de namespace; falha segura se o campo não existir (filtro não acha nada, `Available=true`/`Namespaces=[]`, nunca mistura cluster errado). Credencial via `ElasticsearchCredentialModal.tsx` no menu de perfil (mesmo padrão do Dynatrace). 5 testes unitários via `httptest` cobrindo a lógica da query/parse. **Não validado contra um Elasticsearch real nem clicado na UI** — a sessão Azure AD expirou no meio da implementação (limite de 4h de sign-in frequency por conditional access, `AADSTS70043`), sem JWT novo pro resto da sessão; próximo passo real é uma query `curl` manual contra o índice real pra confirmar os 4 campos assumidos antes de confiar no resultado da triagem.
- [**Estudo/Plano: Integração Zabbix**](ZABBIX-INTEGRATION-PLAN.md) ← 🔬 estudo — API JSON-RPC confirmada e documentada, benefícios mapeados (aba de problems, correlação no Health Check, FinOps para infra self-managed) — **nenhuma fase iniciada, mas todas as perguntas bloqueantes já respondidas (2026-08-19, seção 5)**: Zabbix 7.x, token de serviço só-leitura já existe, host Zabbix = nome do nó K8s (correlação determinística sem heurística), "integração K8s" é Zabbix Agent clássico por VM sem noção de pod/namespace (sem sobreposição com Prometheus/Dynatrace no nível de workload). Fase 4 (correlação no Health Check) realinhada pro padrão leve validado pela integração Spinnaker (`SpinnakerEnricher` — enriquecimento opt-in por cluster, sem aba dedicada). Próximo passo natural: Fase 1 (`internal/zabbix/`)
