...
- [**Plano: Maturidade do Relatório de IA do Health Check**](HEALTHCHECK-AI-REPORT-MATURITY-PLAN.md) ← ✅ Fase 1 concluída (prompt estruturado com emoji de severidade/citação verbatim/3 baldes de urgência; heurística crônico/agudo via novo histórico SQLite `health_check_event_history`; métricas CPU/memória do Dynatrace não são mais descartadas). Fases 2-4 pendentes: (2) wiring do `NodeChecker` (hoje órfão — implementado mas nunca chamado pelo Orchestrator), (3) comparação request-vs-uso-real via Prometheus (adaptando `finops/prometheus_enricher.go`), (4) aba Relatório dedicada
