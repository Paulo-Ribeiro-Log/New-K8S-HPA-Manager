...
- `resource_kind` (`"secret"`/`"configmap"`) + `resource_name` + `resource_subtype` (Type do Secret, vazio pra ConfigMap) — mesma convenção `source_type`/`source_name` já usada em `DependencyRecord`. Resultado mostra as 4 colunas Serviço/Cluster/Namespace/Fonte já usadas na busca reversa normal: "Serviço" vira chave (`text-sm font-semibold`, **sem** badge) + valor mascarado por padrão (`••••••••`) com botão-olho por linha (revelar mostra decodificado, com link pra alternar pra base64) + botão de copiar; "Fonte" vira `secret: X` ou `configmap: X`.
- RBAC: `GET /search-secrets`, `DELETE /secret-data` e também `POST /scan`/`POST /scan/:cluster` (que passam a persistir conteúdo de Secret/ConfigMap) atrás de `RequireSREGroup()` — **nota**: nesse código `RequireSREGroup()` é hoje um no-op (`internal/web/middleware/rbac.go:68-73`, só `c.Set("isSRE", true)`), não restringe nada além de JWT válido (mesmo comportamento documentado em "RBAC Azure AD" abaixo); aplicá-la mesmo assim é a marcação correta caso a checagem real seja reativada no futuro.
