...
- [**Plano: Modo Triagem no Health Check**](HEALTHCHECK-TRIAGE-MODE-PLAN.md) ← 🟡 Fases 1 (backend), 2 (frontend) e 4 (ignore-lists) implementadas — Modo Triagem já é usável ponta a ponta com supressão de ruído configurável, sem validação ao vivo no navegador ainda. Nasceu da conversa sobre integração Zabbix: em vez de varrer o cluster ponto a ponto, resolve primeiro quais namespaces têm problema sinalizado por Dynatrace/Prometheus (Zabbix depois, Fase 5) e só then varre a fundo esse escopo reduzido — mantém a Varredura Completa de hoje como modo alternativo (não substitui, perde detecção de postura/higiene se substituísse). "Grafana" confirmado como não sendo integração nova (mesmos alertas do Prometheus, cliente já existe em `internal/monitoring/alerts`, nunca ligado ao Health Check). Granularidade por namespace na Fase 1 (zero mudança nos `*_checker.go` de resultado, que já aceitam `namespaces []string` filtrado — confirmado na prática) — granularidade por workload fica de fase futura condicional. `HealthCheckRequest.TriageMode` + `TargetSource` (`internal/healthcheck/target_resolver.go`, implementações Dynatrace/Prometheus) resolvem o escopo antes do fallback `getAllNamespaces`; `DynatraceChecker.CheckAll` passou a retornar `error` (só pra essa distinção Available/vazio, único call site atualizado). Achado real corrigido durante a implementação: `internal/web/handlers/healthcheck.go` só populava credenciais Dynatrace do usuário quando `CheckDynatrace`/`CheckOneAgentSignals` estava marcado — sem o `|| TriageMode` adicionado, o Modo Triagem nunca teria credencial Dynatrace sozinho (falharia silenciosamente como "fonte não configurada"). Sem validação ao vivo ainda (só testes unitários + build/vet/suíte existente + type-check/lint do frontend, incluindo teste dedicado pro risco #2 da seção 4 do plano — fonte disponível-vazia vs. indisponível). Frontend: `HealthCheckingTab.tsx` ganhou o card "Modo de Verificação" (toggle Triagem Rápida/Varredura Completa, `false` por padrão) e `HealthReportTab.tsx` ganhou a seção "Escopo da Triagem" com o painel farol por fonte (cinza/verde/âmbar conforme `TriageSourceStatus`). Backlog registrado em 2026-08-20 (a partir da revisão de um script Python de referência de outra ferramenta interna, não portado literalmente): Fase 3 — Elasticsearch/ELK como fonte de erro de log, contorna o mesmo bloqueio de Grail/Platform documentado na seção Dynatrace acima — desbloqueada em 2026-08-20 (esta empresa confirmou operar ELK de verdade, pipeline Fluentd, credencial de leitura já existe, acesso direto ao Elasticsearch sem proxy Kibana; nenhum client Elasticsearch existe nesta app ainda, código não iniciado); Fase 4 — ignore-lists por nome de alerta/trigger/problem (`TriageIgnoreConfig`), gap real confirmado no `FilterManager` atual (`internal/healthcheck/filters.go`), que só cobre postura de recursos K8s, não nome de sinal externo — desbloqueada e implementada no mesmo dia: `TriageIgnoreManager` (`internal/healthcheck/triage_ignore.go`, persistência própria em `triage_ignore.json`, mesmo padrão do `FilterManager` mas lista de entradas tipadas por `Source` em vez de 4 slices paralelas como o desenho original sugeria) filtra `DynatraceTargetSource`/`PrometheusAlertsTargetSource` antes de popular `Namespaces`/`Reasons`; UI nova aba "Sinal Externo (Triagem)" dentro do `FiltersManagementModal.tsx` existente (abas manuais, nunca shadcn `<Tabs>`) em vez de modal irmão separado.
