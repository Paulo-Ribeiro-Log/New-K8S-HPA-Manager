...
- [**Plano: Teste de Latência sob Demanda**](LATENCY-METRICS-PLAN.md) ← ✅ Fases 1-5 concluídas (teste ativo ponta a ponta + guardrails + contexto histórico DT/Prometheus complementar — 2 suposições não validadas contra ambiente real, ver plano), Fase 6 em diante (aba fleet-wide, correlação Health Check) opcional/pendente — ferramenta ativa (botão "Executar Teste", aba `latency-test`/`LatencyTestTab.tsx`): pod efêmero + `curl` via exec (reaproveita `execCmdInPod` do conntrack) mede latência real na hora, SSE de progresso, badge DT/Prom com P95/P99 histórico ao lado do resultado
