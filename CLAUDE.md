...
- [**Plano: Dynatrace — Diagnóstico Acionável**](DYNATRACE-DIAGNOSTICS-PLAN.md) ← ✅ Fase 1 concluída (unificação de regras threshold→ação) — pacote novo `internal/actionrules/` é fonte única de verdade pra "essa métrica está ruim, o que fazer" usado por `generateActionItems` (dynatrace.go), `evaluateRisk`/OneAgent Signals e `severityLabel` (achado extra: um 4º lugar com o mesmo threshold divergente, não previsto no plano original); latência convergida pra P95 em todos (era P90 no OneAgent Signals); `OneAgentThresholds` virou alias de `actionrules.Thresholds` (zero duplicação); **CPU throttle era métrica quebrada, não só threshold divergente** — `builtin:containers.cpu.throttlingTime` não existe mais no Dynatrace (404 testado ao vivo), substituto óbvio por nome (`throttledMilliCores`) tem entidade primária errada (`CONTAINER_GROUP_INSTANCE`, não `CLOUD_APPLICATION`), métrica certa é `builtin:kubernetes.workload.cpu_throttled` (validada com dado real: 1.3-13.9 mCores num workload de produção) — campo renomeado de `CPUThrottlePct` pra `CPUThrottleMilliCores` (nunca foi %, não existe métrica nativa de "% throttled" no catálogo DT); `latencyBreachThresholdMs` (500ms, Fase 7 do plano de latência) **deliberadamente não migrado** pro threshold compartilhado (5000ms) — são perguntas diferentes ("corrobora um DT Problem já aberto?" vs "isso sozinho já alerta?"), migrar teria revertido uma decisão já tomada com o usuário; testes novos em `internal/actionrules/rules_test.go`, `go test ./internal/... -race` completo passa. Fase 2 (navegação real das sugestões — clique leva pro HPA/Deployment já filtrado) e Fase 3 (distributed tracing — achado: endpoint clássico 404 e Grail 403 neste tenant, provavelmente exige OAuth2 em vez do `Api-Token` clássico, decisão do usuário pendente sobre investir nisso) aguardando
