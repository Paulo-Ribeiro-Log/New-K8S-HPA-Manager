...
**Vertex AI via WIF SSO (Workforce Identity Federation)**: Campo `GeminiWifLoginURL` em `UserTokensStore` armazena a URL de login SSO corporativo fornecida pelo TI (formato `https://auth.cloud.google/select-session?...&wiffid=...`). A app exibe um botão "Abrir Login SSO" em `AISettingsTab.tsx` que abre a URL no browser (`window.open`). Após autenticar no browser, as ADC são atualizadas automaticamente pelo gcloud do Windows. **Não há integração direta backend** — é uma URL de conveniência. Após autenticar, clicar em "Testar Conexão" para validar o ADC. Combinação típica: `GeminiAuthMode = "vertex"` + WIF Login URL configurada + `GeminiVertexProject` definido.

**Modelos Gemini**: padrão `gemini-2.5-flash` (Free tier: 15 RPM / 1M+ tokens/dia). Configurável via campo `GeminiModel` no `AISettingsTab`. Modo Vertex AI não aceita todos os modelos do AI Studio — usar modelos disponíveis no endpoint `aiplatform.googleapis.com`.
...
| WIF SSO: "Testar Conexão" falha após login no browser | ADC pode não ter sido atualizado ainda — aguardar alguns segundos e tentar novamente. Verificar se `~/.k8s-hpa-manager/google_adc.json` foi atualizado. Se não, o gcloud do Windows pode estar gravando em `~/.config/gcloud/` — copiar manualmente para o caminho da app |
| WIF SSO: botão "Abrir Login SSO" não aparece | `GeminiWifLoginURL` não foi salvo — verificar se o campo está preenchido e o formulário foi salvo. O campo é exibido apenas quando `geminiAuthMode === "vertex"` em `AISettingsTab.tsx` |
