...
- [**Plano: Maturidade do Relatório de IA do Health Check**](HEALTHCHECK-AI-REPORT-MATURITY-PLAN.md) ← ✅ Fases 1-2 concluídas (prompt estruturado com emoji de severidade/citação verbatim/3 baldes de urgência; heurística crônico/agudo via novo histórico SQLite `health_check_event_history`; métricas CPU/memória do Dynatrace não são mais descartadas; `NodeChecker` — antes órfão, implementado mas nunca chamado pelo Orchestrator — agora roda via checkbox "Capacidade dos Nós" e alimenta o prompt com a tabela de utilização por nó). Fases 3-4 pendentes: (3) comparação request-vs-uso-real via Prometheus (adaptando `finops/prometheus_enricher.go`), (4) aba Relatório dedicada
