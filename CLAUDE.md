...
**Barra de status** (linha azul `#007acc`, altura 20px): exibe posição do cursor `Ln X, Col Y` (capturado em `onDidChangeCursorPosition`), linguagem Monaco e `UTF-8`. Contém toggles inline de **auto-save** (debounce 1,5s após keystroke, `localStorage["ce_autosave"]`) e **format on save** (formata com o formatter da linguagem antes de gravar em disco, `localStorage["ce_format_on_save"]`); ambos persistidos entre sessões.

**Ctrl+P Quick Open**: paleta de arquivos estilo VSCode; overlay sobre a área do editor com `absolute inset-0 z-50`; requer `relative` no container pai; `quickOpenFiles = useMemo(() => flattenTree(tree).filter(...))` filtra em tempo real; registrado no Monaco via `addCommand(2048|46)` e via `document.addEventListener("keydown")` global para quando o Monaco não está focado.
