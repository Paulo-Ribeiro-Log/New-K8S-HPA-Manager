...
**Botão PR**: header, visível quando `branches?.current !== "main" && !== "master"`; abre `https://github.com/${owner}/${repo}/compare/${encodeURIComponent(branch)}`. `owner`/`repo` são extraídos via `ownerRepo(dir)` em `ListRepos` (lê `git remote get-url origin` e parseia os dois últimos segmentos do path) — **não** do ID local por split em `-`, que quebrava com owners contendo hífen (ex: `casas-bahia`).
