...
**Botão PR**: header, visível quando `branches?.current !== "main" && !== "master"`; abre `https://github.com/${owner}/${repo}/compare/${branch}`.
...
**LSP (Language Server Protocol)**: `code_editor_lsp.go` gerencia processos `gopls`/`pyright` por repositório via `sync.Map` (key: `repoId/lang`). Sessões inativas > 10min são encerradas; cleanup a cada 5min. JSON-RPC via stdin/stdout com header `Content-Length`. Frontend registra providers nativos do Monaco **uma única vez** por sessão (flags `__monacoTSConfigured` e `__monacoGoLSPRegistered`) usando variáveis globais `window.__lspActiveRepoId` e `window.__lspActiveFilePath` para comunicar o arquivo ativo. Polling de diagnósticos a cada 2,5s via `setModelMarkers`. `gopls` esperado em `~/go/bin/gopls` ou PATH; `lspVersionRef` incrementado a cada `updateTabContent` e troca de aba. **Nunca registrar `registerCompletionItemProvider("go", ...)` mais de uma vez** — flag global previne duplicação.
