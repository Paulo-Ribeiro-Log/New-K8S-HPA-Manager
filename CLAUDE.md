...
- [**Plano: Teste de Latência sob Demanda**](LATENCY-METRICS-PLAN.md) ← ✅ Fases 1-6 concluídas (teste ativo ponta a ponta + guardrails + contexto histórico DT/Prometheus + protocolo ICMP/HTTP/HTTPS via `nicolaka/netshoot` + alvos multi-cloud AWS + persistência estruturada + grafo de topologia reaproveitando Cytoscape.js do Service Mesh, aba "Topologia" dentro de `LatencyTestTab.tsx`); só falta a Fase 7 opcional (correlação Health Check) — ferramenta ativa (botão "Executar Teste", aba `latency-test`/`LatencyTestTab.tsx`): pod efêmero + curl/ping via exec (reaproveita `execCmdInPod` do conntrack) mede latência real na hora, SSE de progresso, badge DT/Prom com P95/P99 histórico ao lado do resultado
