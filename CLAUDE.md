...
- [**Plano: Descoberta de Prometheus em Clusters GKE**](GKE-PROMETHEUS-DISCOVERY-PLAN.md) ← Fases 1-4 concluídas (parcial na 4, ver ressalvas), Fase 5 validada ao vivo contra `gke_via-gcb-higgs-hlg_southamerica-east1_gke-higgs-hlg` — mecanismo confirmado 100% funcional (auth OAuth2 Bearer via hook `discovery.SetGCPTokenFunc`, URL do Google Cloud Managed Service for Prometheus construída automaticamente a partir do Project ID do context GKE, wireado em `internal/config/kubeconfig.go` não `cmd/web.go` pra reaproveitar bootstrap GKE existente e não fechar o import cycle real entre `cloudprovider/gcp` e `monitoring/discovery`), mas **sem dado de aplicação (HPA/resources) nesse cluster por config de infraestrutura, não bug de código**: o cluster tem um kube-state-metrics completo (`monitoring/prometheus-kube-state-metrics`) sem nenhum `PodMonitoring` apontando pra ele — só o addon reduzido do GKE (`gke-managed-cim`, métricas limitadas, sem HPA/resource requests) está registrado no GMP. Correção (`PodMonitoring` aditivo) identificada e documentada no plano, não aplicada — decisão do usuário de não mexer no cluster nesta sessão. `monitoring/alerts` fica de fora por motivo estrutural (GMP não expõe `/api/v1/alerts`) e os clientes Prometheus internos do FinOps (4º grupo, descoberto nesta fase) ainda não foram cobertos
