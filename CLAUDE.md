...
**Frontend**: header compacto sempre visível (status, % de uso, badge de nós disponíveis). Clique abre `Dialog` (`max-w-2xl`, `max-h-[78vh]`) com 3 abas manuais (div + estado — nunca shadcn `<Tabs>`):
...
Cache de 2min via React Query (`staleTime: 2 * 60 * 1000`). Abas futuras planejadas: **Projeção** (crescimento de nós via SQLite), **Histórico** (gráfico via Prometheus).
