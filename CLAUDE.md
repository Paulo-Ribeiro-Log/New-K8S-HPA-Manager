...
Ver `ACCESS-CHECK-PLAN.md` para o histórico completo de decisões e comandos `az`/`kubectl` de validação.
...
- [**Plano: Teste de Latência sob Demanda**](LATENCY-METRICS-PLAN.md) ← ✅ Fases 1-7 concluídas (Fases 1-6 **validadas ponta a ponta contra cluster AKS real**; Fase 7 validada por teste unitário, não por navegador — ver ressalva abaixo) — teste ativo + guardrails + contexto histórico DT/Prometheus + protocolo ICMP/HTTP/HTTPS via `nicolaka/netshoot` + alvos multi-cloud AWS + persistência estruturada + grafo de topologia reaproveitando Cytoscape.js do Service Mesh (aba "Topologia" dentro de `LatencyTestTab.tsx`) + correlação de breach de latência no Health Check (Fase 7) — ferramenta ativa (botão "Executar Teste", aba `latency-test`/`LatencyTestTab.tsx`): pod efêmero + curl/ping via exec (reaproveita `execCmdInPod` do conntrack) mede latência real na hora, SSE de progresso, badge DT/Prom com P95/P99 histórico ao lado do resultado. **3 bugs reais do contexto histórico (Fase 5) encontrados testando no navegador e corrigidos** (`internal/monitoring/prometheus/client.go`, `internal/web/handlers/latency_history.go`, `internal/dynatrace/latency_metrics.go`): (1) `GetP95Latency`/`GetP99Latency` usavam `http_request_duration_seconds_bucket`/`nginx_ingress_controller_request_duration_seconds_bucket`, nenhuma das duas existe no Prometheus real (cluster roda Istio) — trocado por `istio_request_duration_milliseconds_bucket` com labels reais confirmados via curl direto (`destination_service_name`/`destination_service_namespace`/`reporter="destination"`), mantendo nginx-ingress/genérica como fallback legado; janela do `rate()` de 5m para 30m (serviços de baixo tráfego em HLG têm buracos de minutos sem request, 5m caía com frequência numa janela vazia); (2) o client Prometheus usa "lazy connection" (`TestConnection()` precisa setar `connected=true` antes de `Query()` aceitar rodar) — o código nunca chamava isso, então toda consulta falhava com "not connected" mesmo com o Prometheus 100% acessível; (3) quando nenhuma query candidata achava dado, a função devolvia `(0, nil)` em vez de erro — o zero virava "sucesso" e o `omitempty` no JSON escondia o campo, fazendo o frontend não mostrar nada sem indicar o motivo (corrigido: erro explícito + `omitempty` removido dos campos de latência). Também corrigido `guessServiceNameFromURL` pra aceitar host sem esquema (`url.Parse` sem `http://` na frente trata a string toda como Path, não Host). **Dynatrace**: confirmado direto contra a API real (`GET /api/v2/metrics/builtin:service.response.time`) que a única dimensão dessa métrica é `dt.entity.service` — entidades `SERVICE` nunca tiveram `k8s.namespace.name`/`k8s.workload.name` (diferente de `CLOUD_APPLICATION`, usada em `GetAllWorkloadMetrics` pra CPU/mem no FinOps), a query antiga com `splitBy` nessas chaves sempre falhava com 400; corrigido pra buscar a entidade SERVICE por nome (`entityName.contains`) e consultar a métrica filtrada por `entitySelector=entityId(...)`, validado com dado real (P95=715ms/P99=1072ms numa entidade `WEB_REQUEST_SERVICE` com OneAgent). Achado à parte: os clusters/namespaces do domínio testado (abastecimento-hlg) não têm nenhuma cobertura desse tenant DT (zero entidades `CLOUD_APPLICATION`) — não é bug, é a cobertura real de monitoramento; nesses casos o contexto histórico vem do Prometheus mesmo, corretamente. **Fase 7 — correlação com Health Check** (decisões perguntadas ao usuário antes de codar, plano original não especificava: automático em todo scan, não sob demanda; threshold fixo de 500ms de P95, não configurável): lógica DT→Prometheus da Fase 5 extraída de `fetchHistoricalLatencyContext` pro pacote `internal/monitoring/latencylookup` (`Fetch(ctx, dtURL, dtToken, cluster, namespace, workload)`) — necessário porque `internal/healthcheck` não pode depender de `internal/web/handlers` (inverteria a camada); `latency_history.go` virou wrapper fino sobre esse pacote, mantendo só a heurística `guessServiceNameFromURL` (específica do teste ativo). `internal/healthcheck/latency_breach_checker.go` (`EnrichWithLatencyBreach`) roda logo após `Correlate()` no orchestrator, só pros itens com `DTProblems` não-vazio (custo de N lookups extras por scan proporcional a "workloads com DT Problem ativo", não ao total do cluster — filtro que vem da leitura literal do texto original da Fase 7, "DT Problems abertos no mesmo workload"); semáforo de concorrência 6 (mesmo padrão de scans de frota já usado em nodepool/access-check) + timeout de 30s pro enriquecimento inteiro, best-effort; escalada `LatencyBreach && FinalSeverity < Critical → Critical` (mesmo espírito da escalada K8s↔DT já existente), reordena pelo mesmo critério de `Correlate` depois. `CorrelatedHealthItem` ganhou `LatencyP95Ms`/`LatencyP99Ms` (sem `omitempty`, mesmo motivo do bug da Fase 5) + `LatencySource`/`LatencyBreach`; frontend (`HealthCheckResultsPanel.tsx`) mostra badge vermelho "P95 Nms" no card + seção "Latência histórica" no conteúdo expandido. Lógica de filtro/escalada testável via função de fetch injetável (`latencyFetchFunc`) — 3 testes unitários (`latency_breach_checker_test.go`, passam com `-race`) validam a lógica sem rede real, mas **não foi validada ponta a ponta no navegador** (exigiria Health Check real com DT habilitado + workload com DT Problem + breach simultâneos — combinação não reproduzida no ambiente de teste disponível, mesma limitação de cobertura DT da Fase 5)
- [**Plano: Teste de Kafka sob Demanda**](KAFKA-TEST-PLAN.md) ← ✅ implementado (branch `kfk-whith-auth-tester`) — aba "Teste Kafka", testa TCP/protocolo/SASL/produce-consume/visualizar mensagens contra broker externo (Kafka gerenciado, Event Hub) via Ephemeral Container (`kcat`) anexado a um pod real do Deployment escolhido (não um pod avulso — reflete NetworkPolicy/Istio do workload de verdade); comandos `kubectl`/`kcat` equivalentes documentados passo a passo no plano
- [**Plano: Teste de Banco de Dados sob Demanda**](DATABASE-TEST-PLAN.md) ← ✅ implementado (branch `db-test-tool`) — aba "Teste de Banco de Dados", cobrindo PostgreSQL/MySQL-MariaDB/MongoDB/Redis via registry de engines (`dbEngines` em `db_test_tool.go`); testa conectividade+autenticação (usuário/senha, connection string, ou sem autenticação — cada fonte pode vir digitada manualmente ou lida de Secret/ConfigMap do K8s por nome via combobox com busca, igual à seleção de cluster/namespace/deployment) e navegação só-leitura opcional (databases/tabelas com colunas, collections com contagem de docs, chaves Redis com TYPE — desce um nível quando "Database" é informado) — **nunca escreve no banco**. Dois modos de execução (`execution_mode`): `pod` (default, Ephemeral Container anexado a um pod real, mesmo mecanismo do Teste de Kafka) ou `local` (roda a mesma imagem do engine num container Docker local no host do servidor via `docker run --rm --network host`, sem tocar o cluster — requer só `docker` instalado no servidor, não os clientes nativos; `--network host` reflete a reachability que o próprio host já tem, não cria acesso novo — cuidado com WSL2, onde a rede é isolada do Windows por padrão). Redis também suporta índice de banco numérico (0-15) e padrão de chaves (SCAN...MATCH) no browse. Não validado ponta a ponta contra bancos reais nesta rodada
