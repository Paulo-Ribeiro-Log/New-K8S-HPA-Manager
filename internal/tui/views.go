...
func (a *App) renderHelp() string {
	...
	sections := []struct {
		...
		{"🖥️  NODE POOLS", [][]string{
			...
			{"C", "Configurar cordon/drain (requer 2 marcados)"},
		...
		{"🔧 CONFIGURAÇÃO CORDON/DRAIN (TECLA C)", [][]string{
			{"F12", "Marcar 2 node pools para sequência (*1, *2)"},
			{"C", "Abrir modal de configuração (requer 2 marcados)"},
			{"Tab/Space", "Navegar e toggle opções no modal"},
			{"A", "Expandir/recolher opções avançadas"},
			{"Enter", "Confirmar e executar sequenciamento"},
			{"ESC", "Cancelar e fechar modal"},
			{"🔒", "Drain requer Cordon habilitado (validação)"},
			{"⚙️", "Opções: grace period, timeout, daemonsets, etc"},
			{"📋", "Preview do fluxo completo (5 fases)"},
		}},
