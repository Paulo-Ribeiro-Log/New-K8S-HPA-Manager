...
func (a *Analyzer) GetProviderStatus(ctx context.Context) *ProviderStatus {
	...
	// Fazer chamada real de teste para detectar erros de quota, autenticação, etc
	checkCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()

	// Prompt mínimo para teste (baixo custo de tokens)
	testPrompt := "Responda apenas: OK"
	_, err := a.provider.Analyze(checkCtx, testPrompt)

	if err != nil {
		status.Available = false
		status.Error = a.parseProviderError(err)
	} else {
