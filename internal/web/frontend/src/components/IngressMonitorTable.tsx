...
import type { IngressSummary } from "@/lib/api/types";
...
// SEL(fixed) | NAME/NS | CLASS | HOSTS | AGE | EDIT(fixed)
const INITIAL_WIDTHS = [28, 200, 100, 200, 70, 28];
...
interface IngressMonitorTableProps {
...
export const IngressMonitorTable = ({
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((i) =>
        ...
        (i.addresses ?? []).some((a) => a.toLowerCase().includes(q))
      );
    }
    if (sortKey) {
      result = [...result].sort((a, b) => {
        let va: string, vb: string;
        if (sortKey === "class") { va = a.ingressClass ?? ""; vb = b.ingressClass ?? ""; }
  ...
  const itemKey = (i: IngressSummary) => `${i.cluster}/${i.namespace}/${i.name}`;
  ...
  const toggleItem = (i: IngressSummary) => {
    const key = itemKey(i);
    const next = new Set(selectedKeys);
    ...
    try {
      await Promise.all(
        selectedObjects.map(async (i) => {
          try {
            await apiClient.delete(`/api/v1/ingresses/${i.cluster}/${i.namespace}/${i.name}`);
            succeeded++;
          } catch { failed++; }
        })
      );
      if (succeeded > 0 && failed === 0) toast.success(`${succeeded} Ingress(es) deletado(s) com sucesso.`);
      ...
      <div ref={rowsContainerRef} className="flex-1 overflow-auto">
        <div className="sticky top-0 z-10 grid font-mono text-[10px] px-3 py-1.5 border-b border-border bg-muted/20" style={{ gridTemplateColumns: gridTemplate }}>
          ...
          <span className="relative overflow-hidden pr-4">
            <SortBtn label="CLASS" colKey="class" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
        ...
        {filtered.length === 0 && !loading && (
          <div className="text-muted-foreground text-xs text-center py-6">
            {searchQuery ? "Nenhum Ingress encontrado para a busca" : "Nenhum Ingress encontrado"}
          </div>
        )}
        {filtered.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-muted-foreground truncate">{item.ingressClass || "—"}</span>
              <span className="text-muted-foreground truncate">
                {hosts.length === 0 ? "—" : hosts[0]}
                {hosts.length > 1 && <span className="text-muted-foreground/60"> +{hosts.length - 1}</span>}
              </span>
        ...
        <div className="flex-shrink-0 border-t border-border">
          {bulkAction === "delete" && (
            <div className="flex items-center gap-2 px-3 py-2 text-xs bg-destructive/10 border-b border-destructive/30">
              <span className="flex-1">Deletar {selectedObjects.length} Ingress(es) permanentemente?</span>
          ...
          {!bulkAction && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-muted/30">
              ...
              <span className="text-xs text-muted-foreground">
                <span className="font-medium text-foreground">{selectedKeys.size}</span> Ingress(es) selecionado(s)
              </span>
              <div className="flex-1" />
