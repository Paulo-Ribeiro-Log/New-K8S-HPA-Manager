...
import type { StatefulSetSummary } from "@/lib/api/types";
...
// SEL(fixed) | NAME/NS | READY | CURRENT | UPDATED | AGE | EDIT(fixed)
const INITIAL_WIDTHS = [28, 400, 80, 80, 80, 70, 28];
...
type STSSortKey = "name" | "ready" | "current" | "updated" | "age";

function SortBtn({ label, colKey, sortKey, sortDir, onSort }: {
  label: string; colKey: STSSortKey; sortKey: STSSortKey | null; sortDir: "asc" | "desc"; onSort: (k: STSSortKey) => void;
}) {
...
interface StatefulSetMonitorTableProps {
...
export const StatefulSetMonitorTable = ({
  ...
  useEffect(() => { setLastUpdated(new Date()); }, [statefulsets]);

  useEffect(() => {
    setSelectedKeys((prev) => {
      if (prev.size === 0) return prev;
      const keys = new Set(statefulsets.map((s) => `${s.namespace}/${s.name}`));
  ...
  const handleSort = (key: STSSortKey) => {
    if (sortKey === key) {
  ...
  const uniqueNamespaces = useMemo(() => {
    ...
    statefulsets.forEach((d) => { if (d.namespace) s.add(d.namespace); });
  ...
  const filtered = useMemo(() => {
    ...
    if (sortKey) {
      result = [...result].sort((a, b) => {
        ...
        switch (sortKey) {
          ...
          case "ready":   va = a.readyReplicas / Math.max(a.replicas, 1); vb = b.readyReplicas / Math.max(b.replicas, 1); break;
          case "current": va = a.currentReplicas; vb = b.currentReplicas; break;
          case "updated": va = a.updatedReplicas; vb = b.updatedReplicas; break;
    ...
    return result;
  }, [statefulsets, searchQuery, sortKey, sortDir]);

  const stsKey = (d: StatefulSetSummary) => `${d.namespace}/${d.name}`;
  const allSelected = filtered.length > 0 && filtered.every((d) => selectedKeys.has(stsKey(d)));
  const someSelected = filtered.some((d) => selectedKeys.has(stsKey(d)));

  const toggleAll = () => {
    if (allSelected) {
      const next = new Set(selectedKeys);
      filtered.forEach((d) => next.delete(stsKey(d)));
      setSelectedKeys(next);
    } else {
      const next = new Set(selectedKeys);
      filtered.forEach((d) => next.add(stsKey(d)));
      setSelectedKeys(next);
    }
  };

  const toggleSts = (d: StatefulSetSummary) => {
    const key = stsKey(d);
    const next = new Set(selectedKeys);
  ...
  const selectedObjects = useMemo(
    () => statefulsets.filter((d) => selectedKeys.has(stsKey(d))),
    [statefulsets, selectedKeys]
  );
    ...
    try {
      await Promise.all(
        selectedObjects.map(async (d) => {
          try {
            if (bulkAction === "restart") await apiClient.restartStatefulSet(d.cluster, d.namespace, d.name);
            else await apiClient.deleteStatefulSet(d.cluster, d.namespace, d.name);
            succeeded++;
          } catch { failed++; }
        })
      );
      const label = bulkAction === "restart" ? "reiniciado(s)" : "deletado(s)";
      if (succeeded > 0 && failed === 0) toast.success(`${succeeded} StatefulSet(s) ${label} com sucesso.`);
    ...
    <div className="flex flex-col h-full border border-border rounded-lg overflow-hidden">
      ...
      <div ref={rowsContainerRef} className="flex-1 overflow-auto">
        <div className="sticky top-0 z-10 grid font-mono text-[10px] px-3 py-1.5 border-b border-border bg-muted/20" style={{ gridTemplateColumns: gridTemplate }}>
          ...
          <span className="relative overflow-hidden pr-4">
            <SortBtn label="CURRENT" colKey="current" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
            <ResizeHandle onResize={(d) => resize(3, d)} />
          </span>
          <span className="relative overflow-hidden pr-4">
            <SortBtn label="UPDATED" colKey="updated" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
        ...
        {filtered.length === 0 && !loading && (
          <div className="text-muted-foreground text-xs text-center py-6">
            {searchQuery ? "Nenhum StatefulSet encontrado para a busca" : "Nenhum StatefulSet encontrado"}
          </div>
        )}
        {filtered.map((sts, index) => {
          const isHealthy = sts.readyReplicas === sts.replicas && sts.availableReplicas === sts.replicas;
          const rowColor = isHealthy ? "text-green-600 dark:text-green-400" : "text-orange-600 dark:text-orange-400";
          const readyColor = sts.readyReplicas < sts.replicas ? "text-orange-600 dark:text-orange-400" : "text-green-600 dark:text-green-400";
          const isSelected = selectedKeys.has(stsKey(sts));
          ...
          return (
            <div
              ...
              onClick={() => onOpenEditor(sts)}
              onKeyDown={(e) => {
                if (e.key === " ") { e.preventDefault(); toggleSts(sts); }
            ...
            >
              <span className="flex items-center" onClick={(e) => { e.stopPropagation(); toggleSts(sts); }}>
              ...
              <span className="truncate pr-1 min-w-0">
                {uniqueNamespaces.length > 1 && (
                  <span className="text-muted-foreground text-[10px] block leading-tight">{sts.namespace}</span>
                )}
                <span className="truncate block" title={`${sts.namespace}/${sts.name}`}>{sts.name}</span>
              </span>
              <span className={readyColor}>{sts.readyReplicas}/{sts.replicas}</span>
              <span>{sts.currentReplicas}</span>
              <span>{sts.updatedReplicas}</span>
              <span className="text-muted-foreground">{sts.updatedAt ? formatAge(sts.updatedAt) : "-"}</span>
              <span className="flex items-center justify-center">
                <span
                  className="text-muted-foreground hover:text-foreground p-0.5 rounded"
                  onClick={(e) => { e.stopPropagation(); onOpenEditor(sts); }}
          ...
          {bulkAction && (
            <div className={`flex items-center gap-2 px-3 py-2 text-xs ${bulkAction === "delete" ? "bg-destructive/10 border-b border-destructive/30" : "bg-blue-500/10 border-b border-blue-500/30"}`}>
              <span className="flex-1">
                {bulkAction === "restart"
                  ? `Reiniciar ${selectedObjects.length} StatefulSet(s) via rollout restart?`
                  : `Deletar ${selectedObjects.length} StatefulSet(s) permanentemente?`}
              </span>
          ...
          {!bulkAction && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-muted/30">
              ...
              <span className="text-xs text-muted-foreground">
                <span className="font-medium text-foreground">{selectedKeys.size}</span> StatefulSet(s) selecionado(s)
              </span>
              <div className="flex-1" />
