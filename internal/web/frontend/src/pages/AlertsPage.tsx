// AlertsPage - Página de detalhes de alertas com ações de mitigação
...
export default function AlertsPage() {
  const { cluster, namespace, hpaName } = useParams<{
    cluster: string;
    namespace?: string;
    hpaName?: string;
  }>();
  ...
  const { data: hpaSpecificAlerts, isLoading: loadingSpecific } =
    useHPASpecificAlerts(cluster || "", namespace || "", hpaName || "", isHPASpecific);
  ...
  const {
    ...
    summary,
    loading: loadingAll,
  } = useAllAlerts(cluster || "", !isHPASpecific);
    ...
    return (
      <div className="p-6">
        <Alert variant="destructive">
          ...
          <AlertDescription>Cluster não especificado</AlertDescription>
        </Alert>
      </div>
    );
  }
