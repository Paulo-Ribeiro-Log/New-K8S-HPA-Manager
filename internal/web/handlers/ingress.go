...
// IngressHandler gerencia as rotas de Ingress
type IngressHandler struct {
	kubeManager    *config.KubeConfigManager
	historyTracker *history.HistoryTracker
}

// NewIngressHandler cria um handler com dependências já existentes
func NewIngressHandler(km *config.KubeConfigManager, ht *history.HistoryTracker) *IngressHandler {
...
// List retorna Ingresss com filtros básicos (cluster obrigatório)
func (h *IngressHandler) List(c *gin.Context) {
	...
	ingresses, err := kubeClient.ListIngresses(c.Request.Context(), namespaces, search, showSystem)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"error": gin.H{
...
// Get retorna o manifesto completo de um Ingress específico
func (h *IngressHandler) Get(c *gin.Context) {
	...
	kubeClient := kubeclient.NewClient(clientset, cluster)
	manifest, err := kubeClient.GetIngress(c.Request.Context(), namespace, name)
	if err != nil {
...
func (h *IngressHandler) Diff(c *gin.Context) {
	var req ingressDiffRequest
...
func (h *IngressHandler) Validate(c *gin.Context) {
	var req ingressValidateRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, errorResponse("INVALID_REQUEST", fmt.Sprintf("Invalid body: %v", err)))
		return
	}
	...
	result, err := kubeClient.ValidateIngress(c.Request.Context(), sanitizedYAML, req.FieldManager, req.Namespace)
	if err != nil {
		status := http.StatusInternalServerError
...
func (h *IngressHandler) Apply(c *gin.Context) {
	...
	if !req.DryRun {
		if manifest, err := kubeClient.GetIngress(ctx, namespace, name); err == nil {
			before = ingressManifestToHistoryMap(manifest)
		}
	}
	...
	if !req.DryRun && h.historyTracker != nil {
		after := ingressToHistoryMap(result)
		entry := history.HistoryEntry{
			Action:   "apply_ingress",
...
func sanitizeIngressYAML(yamlContent string) (string, error) {
	...
	if err := yaml.Unmarshal([]byte(yamlContent), &obj); err != nil {
		return "", fmt.Errorf("invalid ingress yaml: %w", err)
	}
	...
	// Remover campos gerenciados pelo sistema
	...
	if err != nil {
		return "", fmt.Errorf("failed to marshal sanitized ingress: %w", err)
	}

	return string(cleaned), nil
}

func ingressManifestToHistoryMap(manifest *models.IngressManifest) map[string]interface{} {
	if manifest == nil {
		return nil
	}
...
func ingressToHistoryMap(cm *networkingv1.Ingress) map[string]interface{} {
	if cm == nil {
		return nil
	}
	return map[string]interface{}{
...
func (h *IngressHandler) Describe(c *gin.Context) {
	...
	output, err := kubeclient.ExecuteKubectlDescribe(authArgs, "ingress", name, namespace)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
...
// Delete deleta um Ingress específico
func (h *IngressHandler) Delete(c *gin.Context) {
	...
	if err != nil {
		...
		if apierrors.IsNotFound(err) {
			c.JSON(http.StatusNotFound, gin.H{
				...
				"error": gin.H{
					"code":    "NOT_FOUND",
					"message": fmt.Sprintf("Ingress %s/%s not found", namespace, name),
				},
			})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{
			...
			"error": gin.H{
				"code":    "DELETE_ERROR",
				"message": fmt.Sprintf("Failed to delete ingress: %v", err),
			},
		})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": fmt.Sprintf("Ingress %s/%s deleted successfully", namespace, name),
	})
}
