...
func (h *ConfigMapHandler) Describe(c *gin.Context) {
	...
	output, err := kubeclient.ExecuteKubectlDescribe(authArgs, "configmap", name, namespace)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
...
func (h *ConfigMapHandler) Delete(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			...
			"error": gin.H{
				"code":    "DELETE_ERROR",
				"message": fmt.Sprintf("Failed to delete configmap: %v", err),
			},
		})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": fmt.Sprintf("ConfigMap %s/%s deleted successfully", namespace, name),
	})
}
