...
**Status**: 🟢🟢 **mecanismo de rollback resolvido pros dois ambientes** — PRD via pipeline dedicado (`rollback_type: "explicit"`), HLG via comparação de versão-alvo-de-execução-falha contra versão vigente no K8s (`rollback_type: "implicit"`, seção 0.6, confirmado com o usuário). UI especificada pelo usuário (seção 8), contrato de dados final (seção 5). Login/API confirmados nos dois Gates (`spinnaker-hlg-api`/`spinnaker-prd-api`). **Configuração no Perfil do Usuário especificada pelo usuário e validada ao vivo (seção 10)**: tipo de login (email/matrícula), URLs base do Deck por ambiente (Gate resolvido sozinho via `settings.js`), e seletor de projeto alimentado por `GET /projects` (confirmado — resolve de quebra a hierarquia Project→Applications→nameApp que faltava mapear). Item remanescente antes da Fase 1: generalizar a convenção de nome de pipeline de rollback pra outras squads (item 2 da seção 6). **Revisão 12** (2026-08-18).
