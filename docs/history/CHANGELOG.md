...
### Workloads — aba Nodes (cordon/uncordon, drain com progresso, delete, YAML editável) (Setembro 2026)

Pedido: aba Nodes no padrão da aba Namespaces — lista à esquerda com menu de 3 pontos (Cordon/Uncordon, Drain, Describe, Delete) e, à direita, detalhes + YAML no Monaco com as ferramentas de edição de Deployments/Pods. Backend em `/api/v1/cluster-nodes` (`handlers/cluster_nodes.go`; `/nodes/:cluster/:nodepool/...` já era das telas de Node Pools): listagem de todos os nodes em 3 chamadas (nodes + `metrics.k8s.io` + pods com `resourceVersion=0`, em vez de uma por node como `GetNodesWithMetrics`), YAML sem `status`/campos do servidor, apply via SSA (dry-run = Validar), describe, delete, cordon/uncordon, `permissions` (SelfSubjectAccessReview para `patch/delete nodes` e `create pods/eviction` — Node é recurso sem namespace, fora do RBAC por namespace das outras abas). Toda escrita registra no histórico (`apply_node`, `delete_node`, `uncordon_node` novos; `cordon_node`/`drain_node` existentes) e fica atrás de `RequireSREGroup()`.

Drain da aba: `CheckDrainable` (recusa estrita do kubectl — DaemonSet sem "ignorar", pods sem controller sem force, emptyDir sem "apagar dados", listando os pods) → cordon → `DrainNodeWithProgress` com progresso em SSE sobre o próprio POST (`apiClient.drainClusterNode` lê o stream via fetch; abortar cancela o drain). Melhorias no drain compartilhado com o sequenciamento de node pools (sem mudar o que ele aceita): eviction bloqueada por PDB (429) é tentada de novo a cada 5s até o timeout (antes abortava no primeiro pod), mirror/static pods ignorados, espera trata pod recriado com mesmo nome (UID diferente) como removido e não assume removido em erro transitório. Delete avisa que a VM continua existindo na cloud (mesmo comportamento de kubectl/k9s).

Frontend: `NodesTab` — painel esquerdo com a lista de nodes (filtros por status/node pool, ordenação por CPU/memória/pods); clicar leva ao YAML/detalhes do node no painel direito. Sem node selecionado, o painel direito navega por node (estilo k9s): nodes → namespaces com pods naquele node (`GET /cluster-nodes/:cluster/:name/workloads`, uma listagem de pods por `spec.nodeName`; deployment resolvido pelo ReplicaSet menos o `pod-template-hash`) → deployments com pods no node → pods do node. Os dois últimos níveis são a `DeploymentsTab` embutida com a prop nova `nodeFilter` (filtra a tabela de deployments e o drill-down de pods; sem a prop, nada muda). A visão geral por node pool fica no modal "Visão geral dos nodes". Ações em lote nas duas listas de nodes (esquerda e painel direito), com a mesma seleção e no molde da `DeploymentMonitorTable` (checkbox redondo, "selecionar todos", barra no rodapé com confirmação inline): Cordon/Uncordon via `POST /cluster-nodes/:cluster/schedulable` (5 em paralelo, falha de um não interrompe os outros, histórico por node; ignora os que já estão no estado pedido e avisa quando um pool ficaria sem node Ready aceitando pods) e Drain em lote — o modal de drain aceita vários nodes e drena um de cada vez, parando no primeiro que falhar. `ResourceYamlPanel` — editor YAML reutilizável com desfazer/refazer (histórico real por pausa de digitação), editor/diff, tela cheia, diff lado a lado, dry-run, cancelar, aplicar com confirmação das mudanças e modal de erro; candidato a substituir as cópias do mesmo bloco nas outras abas.
...
- **Fonte:** uma consulta por subscription ao Cost Management (`AmortizedCost`, 30 dias até ontem, `ServiceName = Virtual Machines`, `PricingModel ∈ {Reservation, SavingsPlan}`, agrupado por `PricingModel × Meter`), só leitura. Na subscription real: 28 SKUs / 32 linhas (`F4s v2` R$ 52 mil, `D4s v4` R$ 32 mil, `D8s v4`, `F8s v2`, `F16s v2`, `D2s v4`; Savings Plan em `D8s v5`, `D4a/D4as v4`, `D8ls v5`…). `azure.SKUsFromMeter` converte o medidor em SKU (`F4s v2` → `Standard_F4s_v2`; `D8a v4/D8as v4` vira os dois SKUs; medidores com "Windows"/"Low Priority" são descartados). O parser de linhas do Cost Management foi extraído (`parseCostRows`) e compartilhado com a cobertura por pool.
- **Persistência:** `sku_pricing_coverage` + `sku_pricing_coverage_meta` (`finops-rightsizing.db`; snapshot por subscription; a hora da consulta fica na tabela meta, então uma subscription sem nenhum SKU coberto também conta como "consultada" e o TTL não repete a chamada). Não altera as tabelas estáticas do código (`azureVMTierSpecs`, `fallbackPrices`) — o índice é dado de runtime, consultado por cima delas.
- **Atualização a cada scan:** `persistRightsizingFromReport` (usada pelo scan de Rightsizing e pelo "Analisar" do FinOps) chama `autoRefreshCoverage` antes de sugerir tiers: atualiza a cobertura por pool do cluster e o índice da subscription, **cada um só se tiver mais de 6 h** (o Cost Management limita chamadas), com teto de 60 s, **best-effort** — qualquer erro (sem permissão, VPN, `429`) só é registrado e o scan segue. O botão "Atualizar cobertura" agora também atualiza o índice (e mostra quantos SKUs ficaram nele; falha do índice vira aviso, sem perder a cobertura por pool).
- **Como entra nas sugestões:** (1) **etiqueta na leitura** (`reserved_hint`, como o desempenho e a cobertura): "Reserva na frota" (SKU exato), "Savings Plan na frota" (SKU exato) ou "Reserva da série na frota" (outro tamanho da mesma série — vale com flexibilidade de tamanho); Savings Plan não se propaga por série porque cobre várias famílias. (2) **gêmeos de geração anterior no scan** (`AugmentWithCoveredTwins`): a regra do `SuggestVMTier` só oferece a geração mais nova; agora, quando o SKU de mesma configuração (letras, vCPUs, sufixo e memória) de outra geração **roda coberto na frota** (SKU exato, não só a série), ele é oferecido também, herdando o veredito do irmão e com a razão "Mesma configuração da geração anterior (X), que já roda sob reserva/Savings Plan na frota". Teto de 4 alternativas. Ordem: veredito → cobertura (reserva > Savings Plan > série > nenhuma) → geração mais nova. Só AKS; sem índice as alternativas ficam como estavam. `buildVMAlternative` foi extraída do `SuggestVMTier` (comportamento idêntico, testes existentes intactos).
- **Verificado com pricer e índice reais:** `D4s_v4` sobredimensionado (o exemplo do usuário) — antes só `D2s_v5 (cheaper)`; agora `D2s_v4 [reserva]` + `D2s_v5 [sem cobertura]`. `calculofrete` (F4s_v2, gargalo de memória) — antes só `D4s_v5`; agora `D4s_v4` e `D4s_v3` (reserva) + `D4s_v5` (Savings Plan). Atenção: o `D2s_v4` é mais caro que o `D2s_v5` no preço de tabela (US$ 0,153 × 0,096/h) — só compensa se a reserva tiver capacidade livre.
- **LIMITE principal (dito na própria etiqueta):** o índice vem de uso **observado**: mostra que o SKU já roda sob reserva, **não que há capacidade ociosa**. Reserva ociosa não aparece (não há linhas `UnusedReservation` nessa subscription) — exigiria ler as reservas (`Reservations Reader`/`Savings plan Reader`, hoje `AuthorizationFailed`). Outras limitações: índice da subscription inteira e sem filtro de região (o Cost Management só agrupa por 2 dimensões); reserva de escopo compartilhado (`..._SHARED_1`) pode ser usada por outras subscriptions que o índice não vê; o "melhor caso" dos cenários de troca não credita a cobertura da própria alternativa (e o "pior caso" segue válido).
- **Não exercitado ponta a ponta:** o refresh automático DENTRO do scan (depende de Prometheus/Dynatrace do cluster); foram validados separadamente a consulta real (8,5 s), a persistência/TTL, a geração de alternativas com o pricer real e a UI. Testes: `cost_index_test.go`, `finops_sku_coverage_test.go`, `sku_coverage_test.go`, `finops_coverage_test.go`.

### FinOps — cobertura de reserva / Savings Plan por node pool + aviso ao trocar de SKU (Setembro 2026) ✅

Motivação (usuário: "temos reservas e savings plan"): as sugestões de tier calculam a economia com o preço de TABELA (pay-as-you-go). Num pool coberto por reserva isso engana — a reserva é paga de qualquer jeito, e uma VM de outra série sai da cobertura. Itens 1 e 2 do combinado; o item 3 (efeito real da troca, em R$) foi feito em seguida — ver "Efeito real da troca" abaixo.

- **Achado que motivou:** no `oferta-prd` (custo amortizado, 30 dias) o pool `calculofrete` (54× F4s_v2) está **99% em reserva** — custo efetivo ~R$ 15 mil contra ~R$ 53 mil de tabela — e é justamente o pool para o qual a ferramenta sugere `F4s_v2 → D4s_v5` com "economia" de R$ 13.665/mês. `regionaliza` 99%, `ingress`/`frete`/`ofertasysn` 98-99%, `monitoring` 90% reserva + 10% Savings Plan, `infratoospot` 100% spot.
- **De onde vem:** Cost Management (`POST .../providers/Microsoft.CostManagement/query`, só leitura), que informa o `PricingModel` de cada linha (Reservation / SavingsPlan / OnDemand / Spot). Funciona com o papel de leitura de custos da subscription; **não** exige `Reservations Reader`/`Savings plan Reader` (as APIs `Microsoft.Capacity`/`BillingBenefits` dão `AuthorizationFailed` para a identidade atual — por isso não dá para saber escopo, validade ou quantidade das reservas).
- **Custo AMORTIZADO, não horas nem custo efetivo:** `UsageQuantity` tem unidade que varia por medidor (209 "horas" para 54 nodes) e no `ActualCost` uma hora coberta por reserva pré-paga custa ZERO (some da conta: reserva = R$ 0, sob demanda = R$ 357). Fatia = participação no custo amortizado, então a de reserva aparece um pouco menor que a de horas.
- **Como funciona:** `POST /api/v1/finops/pricing-coverage/refresh` (`{cluster, days?}`, padrão 30, 7-90; janela termina ONTEM — o dia corrente está incompleto; só AKS) lê o `nodeResourceGroup` do managed cluster (não dá para derivar do nome), consulta o custo por `PricingModel` × `ResourceId` e agrega por node pool a partir do nome do VMSS (`aks-<pool>-<id>-vmss`). `ARMClient.PostJSON` repete em `429`/`503` respeitando `Retry-After` (a API de custos limita chamadas; visto um `429` na 1ª tentativa). Snapshot por cluster em `pool_pricing_coverage` (`finops-rightsizing.db`, fora da limpeza de semântica antiga). Botão "Atualizar cobertura" ao lado de "Medir desempenho de CPU" (desabilitado fora de AKS).
- **Nas sugestões** (montado na LEITURA, como o desempenho medido — atualizar a cobertura não exige reanalisar): o card do pool ganha barra + "Reserva 99% · Sob demanda 1% (30d · há N)"; cada alternativa de SKU de um pool com reserva ≥ 20% ganha um aviso âmbar: outra série → a reserva não cobre a nova VM (nodes trocados seriam sob demanda); mesma série → vale com flexibilidade de tamanho, mas consome a reserva proporcionalmente. Savings Plan ≥ 20% → mantém o desconto (vale para várias famílias) mas o compromisso por hora segue cobrado. **Redação condicional de propósito:** a reserva "só não vira desperdício se outras VMs do mesmo SKU dentro do escopo dela a absorverem" — sem ler a reserva não sei o escopo. Pool spot (reserva não se aplica) e pool sem cobertura consultada não recebem aviso. **Informativo:** não altera `monthly_savings_brl` nem o veredito.
- **Efeito real da troca (item 3):** a economia mostrada é o preço de TABELA, que só vale se a reserva liberada for reaproveitada. Como a reserva é paga de qualquer jeito, cada alternativa de um pool com reserva ≥ 20% (e Savings Plan < 20%) ganha um intervalo em R$/mês, sinal de economia (`finops.AssessSwapScenarios`, montado na leitura): **melhor caso** = a economia de tabela (reserva absorvida por outras VMs do mesmo SKU no escopo dela); **pior caso** = a reserva fica ociosa — outra série: os N nodes da alternativa saem sob demanda menos a parte sob demanda de hoje (o custo efetivo é da janela inteira, normalizado por `HoursPerMonth/24/window_days`); mesma série menor: a parte reservada não encolhe, só a fatia sob demanda economiza a variação de preço. Real (`calculofrete`, 52× F4s_v2 → D4s_v5): melhor **+R$ 13.665** / pior **−R$ 37.009** por mês; `F4s_v2 → F2s_v2` em `frete`/`ingress`: melhor R$ 984 / pior ≈ R$ 4-6. **Sem chute:** não calcula (só o aviso de texto) com Savings Plan relevante (o plano acompanha a troca), moeda ≠ BRL, sem nodes/preço/câmbio, ou mesma série para tamanho MAIOR. Premissas explícitas na nota (tooltip): nodes atuais e pool inteiro trocado. Decisão do usuário: faixa com 2 cenários e sem estimar a absorção (exigiria outra consulta ao Cost Management, sujeita a `429`, e só enxerga a subscription, não o escopo real da reserva). O câmbio só é buscado quando há pool com reserva (a consulta pode ir à rede). Frontend: bloco "Melhor caso / Pior caso" dentro do aviso.
- **Limites conhecidos:** só AKS; não lê quais reservas existem nem quando vencem; janela até ontem (defasagem do Cost Management); fatia de custo ≠ fatia de horas; o pior caso supõe o pool inteiro trocado com o nº de nodes atual.
- **Validação:** testes (`cost_coverage_test.go`, `coverage_test.go`, `finops_coverage_test.go`, `finops_pricing_coverage_test.go`) com os números reais do oferta-prd + `go test ./internal/... -race`; chamada real do endpoint contra o Cost Management do `oferta-prd` (6 s, bate com o protótipo manual); no navegador (instância isolada, HOME temporário/porta 8092): botão, barras, 4 avisos, toast e zero erros de JS. Nada criado nem alterado na Azure nem nos clusters.
...
- **O que a documentação da Microsoft diz** (páginas das séries Fsv2 e Dv4/Dsv4): o SKU **não fixa o processador**. Fsv2 roda em Xeon Skylake 8168, Cascade Lake 8272CL, Ice Lake 8370C ou Emerald Rapids 8573C ("sizes may run on any of the listed processors"); Dsv4 em Cascade Lake, Ice Lake, Sapphire Rapids 8473C ou Emerald Rapids. As duas anunciam turbo all-core de 3,4 GHz e hyper-threading. Diferença estrutural real: F = 2 GiB/vCPU, D = 4 GiB/vCPU. (A página de ACU da Microsoft hoje redireciona para outra página — não reconfirmei os números de ACU, por isso nenhum valor de tabela foi embutido.)
- **O que foi MEDIDO** (HLG, 5 clusters, `nicolaka/netshoot` com benchmark single-thread): em **todos** os nodes, F4s_v2 e D4s_v4 reportam o **mesmo** Xeon Platinum 8370C @ 2,8 GHz (Ice Lake). CPU geral (loop de inteiros): F 18,75 × D 19,02 → **0,99× = empate** (a própria medição varia 6–27% entre amostras). **Criptografia é outra história:** RSA-2048 1284 × 2638 assinaturas/s (**D 2,05× mais rápido**), SHA-256 337 × 1036 MB/s. Causa (flags de `/proc/cpuinfo`): o D4s_v4 expõe `avx512ifma sha_ni vaes vpclmulqdq` e o F4s_v2 **não** (só `adx avx2 avx512f`). Hipótese (inferência, não medida): a série Fsv2 precisa rodar em 4 gerações de CPU e o hypervisor expõe o conjunto comum de instruções. Conclusão: "F é mais rápida" **não se confirma** neste ambiente — para código genérico é equivalente; para TLS/criptografia (ingress, service mesh) o D4s_v4 é bem melhor. Só foi medido HLG (Ice Lake); em PRD os hosts podem ser outros.
- **Como funciona:** `POST /api/v1/finops/perf-benchmark` (SRE) cria 1 pod efêmero por node amostrado (espalhado pelo pool; 250m de CPU, sem privilégios/root/capabilities/token, sem sidecar Istio, tolera qualquer taint, fixado no node por hostname), roda `finops.PerfBenchScript` (5× loop de inteiros em Python, melhor amostra + espalhamento como sinal de ruído; RSA-2048 do OpenSSL, melhor de 3; extensões de CPU) e remove o pod. Guarda em `node_perf_benchmarks` (`finops-rightsizing.db`; upsert por cluster+node, poda 90 dias, migração idempotente da coluna `cpu_features`). Ignora nodes Windows/cordonados/NotReady/com taint `*/unschedulable` (ex: `remediator.kubernetes.azure.com/unschedulable`, node em reparo) e com pressão de disco/memória/PIDs (a imagem tem ~500 MB — visto um pod `Evicted … DiskPressure`). Erros por node trazem o motivo real do scheduler (`0/5 nodes are available: 1 Insufficient cpu`). `GET /finops/perf-benchmark?cluster=` devolve as medições do cluster e o quadro por SKU da frota.
- **Nas sugestões** (`finops/vm_perf.go`, aplicado **na leitura** em `nodePoolTierResponses`, então uma medição feita depois do scan aparece sem reanalisar): cada alternativa ganha `perf` com `CPU por thread ×N` (faixas: ±5% equivalente — a margem de ruído; 90–95% levemente mais lento; <90% mais lento), `TLS/RSA ×N` **separado** (mesmo processador pode render igual em código genérico e 2× diferente em cripto) e `lost_features` (extensões que o SKU atual tem e a alternativa não expõe). Sem medição: "sem medição — use Medir desempenho de CPU" (nunca um número inventado); tamanho não medido da mesma série herda por thread, marcado "inferido da mesma série" (`SeriesKey`, case-insensitive). **Escopo:** o SKU atual usa a medição dos nodes **do próprio pool** ("Medido neste pool"); se só existir de outros clusters, aparece em âmbar "Medido só em outros clusters" e a nota avisa que o processador daqui pode ser outro. Informativo: não altera `verdict`/`reason`. Frontend: `components/finops/PerfBenchmark.tsx` (botão "Medir desempenho de CPU" ao lado de "Reanalisar agora", diálogo com aviso do que será criado, resultado por node e quadro por SKU).
- **Não mede** rede, disco nem latência de aplicação: é CPU por thread. O tempo de resposta real também depende de I/O, rede e throttling por limit.
- **Validação:** testes (`vm_perf_test.go`, `finops_perf_test.go`, `finops_perf_benchmarks_test.go`) + suíte `go test ./internal/... -race` verde; medição real em 5 clusters HLG (nenhum pod/configmap de teste restou); fluxo completo no navegador (Chrome/Playwright) contra nodes reais: antes "Medido só em outros clusters", depois da medição pelo diálogo "Medido neste pool", com `×0,98` e `TLS/RSA ×0,48` — zero erros de JS. Instância de teste isolada (HOME temporário, porta 8092); o servidor do usuário não foi tocado.
- **Verificado e descartado (nada a corrigir):** o registro de node pools tem 13 pools com `Standard_f4s_v2` (minúsculo) e a API de preços da Azure diferencia caixa, mas `normalizeVMSize` (`azure_pricing.go`) já coloca a letra da família em maiúscula antes de `GetPrice`/`GetVMSpecs`/fallback. Confirmado com o `AzurePricer` real: `Standard_F4s_v2` e `Standard_f4s_v2` dão o mesmo preço (fonte `api`), as mesmas specs e as mesmas alternativas em `SuggestVMTier`. (Uma 1ª leitura, testando só `vmSizeToSKUName` isolada, apontou um bug que não existe.)

### FinOps — desperdício/utilização inflados em workloads multi-réplica: request era o TOTAL, uso/recomendado era POR POD (Setembro 2026) ✅

Relato (usuário): no maior cluster (`akspriv-oferta-prd`, 72 workloads) um item de custo R$ 10.846 aparecia com "economia de R$ 10.758" (99%) — implausível. Investigado nos dados reais (`finops-report-cache.db`): era o workload `frete-hub` (70 pods).

- **Causa raiz** (`internal/finops/calculator.go`): `rawWorkload` soma o request de todos os pods (`+=`), e o `FinOpsWorkload` herdava essa SOMA — mas o modelo (`"request configurado (por pod)"`) e o frontend (`DashboardTab`: `cpu_request_millis * pods`; `NodePoolsTab`: `avg * pods`) esperam **por pod**, e o uso histórico/recomendado (P95 = máximo entre pods; avg = média entre pods; Prometheus **e** Dynatrace — conferido: `avg × pods ≈ uso atual total` nas duas fontes) é de **um pod**. `calculateWaste` e `verdictFromPrometheus` comparavam `56000m` (70 pods) com `60,89m` (1 pod): `(56000−60,89)/56000 ≈ 99,9%`; com memória `98,5%` → média 99,18% × R$ 10.846,55 = **R$ 10.757,96** (o número exibido, batendo até os centavos). Efeitos colaterais do mesmo erro: `oom_risk` nunca disparava em workload multi-pod (P95 de 1 pod nunca chega a 95% do request total), memória "ociosa" fantasma marcava `superprovisioned`, `poolMeta.maxCPUReqMillis` ("maior request individual") virava o total → `insufficient_for_largest_workload` sempre verdadeiro, e o Dashboard multiplicava por `pods` duas vezes.
- **Segundo ponto, mesma origem** (`finops_rightsizing.go`): a demanda por node pool somava o recomendado/P95 **de um pod** por workload, sem multiplicar pelo nº de réplicas → utilização do pool `calculofrete` (54 nodes) aparecia como "CPU 0,8% / Mem 3,8%" enquanto os nodes reais estavam a 16%/65% — base das sugestões de downsize de VM (F4s_v2 → F2s_v2, −50%) sem lastro. Agora `valor por pod × pods`.
- **Correção:** `FinOpsWorkload.CPURequestMillis/MemRequestMi/CPULimitMillis/MemLimitMi` passam a ser **por pod** (total ÷ pods) na construção; a fração de custo continua usando o request total (`rawWorkload`). Frontend sem mudança (já assumia por pod). Texto do prompt de IA agora diz "request por pod … (xN pods)".
- **Efeito nos dados reais** (recálculo do cache): `frete-hub` R$ 10.758 → **R$ 5.010** (só a CPU sobra: 800m de request p/ ~51m de P95; memória está a 89% do request); economia potencial do cluster R$ 46.201 → **R$ 23.009**; utilização de `calculofrete` 0,8%/3,8% → **39,6%/95,7%** (coerente com os nodes reais). O que ainda é grande é real: CPU request 16× acima do uso.
- **Dados antigos descartados uma vez:** relatórios em cache e snapshots de Rightsizing gravados com a semântica anterior ficariam errados na tela; `purgeFinOpsDataIfStale` (`internal/storage/finops_data_version.go`, `PRAGMA user_version`) limpa `finops_report_cache`, `workload_recommendations`, `nodepool_tier_suggestions` e `node_usage` na primeira subida com este binário — é dado derivado; um novo "Analisar" por cluster refaz tudo. Roda só uma vez.
- Testes: `calculator_per_pod_test.go` (request/limit por pod, custo alocado inalterado, desperdício do `frete-hub` ≈ R$ 5.010, `oom_risk` multi-pod, workload de 1 pod inalterado) e `finops_data_version_test.go` (limpa só uma vez, não apaga dado novo).
...
- **Bug de preço (o mais grave):** `DiskPricer.queryPricingAPI` escolhia o *menor* preço da unidade `1/Month`, que em Standard SSD/Premium SSD é a tarifa acessória **"Disk Mount" (US$ 1,82)** e não o disco (P10 US$ 34,05; E10 US$ 17,92 em brazilsouth) — subestimava esses discos em ~10–20x em TODO o FinOps (PVCs, disco de OS de node pool, RG de dados). Corrigido com `pickRetailPrice` casando o medidor exato `"<tier> LRS Disk"`; o cache SQLite foi trocado (`disk_pricing_cache_v2`, a v1 é apagada) pra não servir os valores errados. Efeito visível: o RG `rg-oferta-data-prd` foi de R$ 39,7 mil pra R$ 50,0 mil/mês. **Totais de storage de execuções anteriores estão subestimados.**
- **Coleta via ARM REST** (`internal/cloudprovider/azure/arm.go` + `rg_resources.go`, mesmo cliente da descoberta de desatachados) no lugar de 3 chamadas `az` CLI. Cada disco traz estado (`Attached`/`Unattached`/`Reserved`), VM dona, tier cobrado (`properties.tier` ou o que cobre o tamanho) e IOPS/throughput provisionados.
- **VM desalocada não cobra compute:** disco em estado `Reserved` denuncia a VM dona desalocada (o ARM só entrega power state por VM via instanceView — evitado). Antes ela era precificada como ligada (`rg-abastecimento-data-hlg`: R$ 2.206 → R$ 1.714/mês).
- **`PremiumV2_LRS` era precificado como Premium SSD** (o mapeador casa "premium" por prefixo); agora usa a tabela de preço de `disk_price_table.go`.
- **Ofertas de resizing** (`internal/finops/data_resources_resizing.go`): de *inventário* (sempre) — disco desatachado, disco de VM desalocada, Premium SSD em HLG → Standard SSD — e de *uso real* sob demanda (`GET /finops/data-resources?analyze=true&days=14`, Azure Monitor, cache de 30 min): VM (CPU/memória, reaproveitando `SuggestVMTier`; VM de 2 vCPU recebe oferta Burstable escolhida pela memória de pico) e Flexible Server (CPU/memória/storage; storage só pode crescer). Sem métrica de memória nunca se afirma que a VM está sobrando. Ofertas são sugestões com a evidência ao lado — nada é aplicado.
- **Flexible Server (PostgreSQL/MySQL) — custo completo, não só compute.** A nota antiga ("Só compute (vCore/hora) — custo de storage provisionado não coberto") deixava o número da tela enganoso: num servidor Burstable pequeno o storage custa MAIS que o compute (`pgsh-adanalytics-1`: `B1ms` com 512 GB P20 = compute R$ 131 + storage R$ 575). Agora `ListRGFlexServers` (ARM) traz `storageSizeGB`/tier/IOPS/HA e o custo soma compute + storage provisionado (US$ 0,2185/GB/mês em brazilsouth, medidor "Storage Data Stored"), em dobro quando há HA (por documentação — nenhum servidor real da frota tem HA ligado, então não foi validado com dado real). Backup acima do incluído e IOPS extra continuam fora do total, e a nota diz isso.
- **Bug de preço de compute Burstable:** a Retail API grava o skuName em caixa alta (`B1MS`) e o filtro `contains(skuName, 'B1ms')` diferencia maiúsculas — 14 dos 20 Flexible Servers reais (todos os `B1ms`/`B2s`) ficavam **sem preço nenhum**. Agora filtra pelo produto "Burstable" e casa o SKU em Go sem diferenciar caixa. Compute e storage são consultados de forma independente: falha num não esconde o outro. A oferta de resizing compara compute com compute (o storage não muda com a troca de SKU).
- **Resolução do RG de dados pelo nome do cluster + busca entre subscriptions** (`internal/finops/data_rg_resolve.go`). Convenção confirmada: `rg-<nome do cluster sem prefixo aks*- e sem sufixo -admin/-<env>>-data-<env>` (ex: `akspriv-abastecimento-hlg-admin` → `rg-abastecimento-data-hlg`), aplicada ao context do kubeconfig — cluster que só existe no kubeconfig (sem entrada em `clusters-config.json`) também funciona. Fallback: o RG de app registrado no config (`rg-<x>-app-<env>` → `-data-`), porque a convenção pelo nome falha quando o RG não tem o nome do cluster (`akspriv-mktplacecarrinho-prd` → RG real `rg-mktplace-data-prd`). **Achado real:** o RG de dados pode estar em OUTRA subscription que a do cluster (`cshub-hlg`: cluster em BACKOFFICE, RG em ONLINE; `mktplacecarrinho-prd`: cluster em ONLINE 2, RG em ONLINE) — o coletor só procurava na subscription do cluster e respondia "RG não encontrado". Agora tenta a subscription do cluster e, se não achar, procura em todas as acessíveis (cache de 1h; "não existe" 5 min); nome existente em várias subscriptions sem ser a do cluster é erro de ambiguidade, nunca palpite. Resultado nos 25 contexts AKS do kubeconfig: 22 → **24** com RG de dados (o `mktplacecarrinho-prd` passou a mostrar 45 recursos / ~R$ 18 mil por mês, antes invisíveis); só `marketplace-hlg` não tem RG de dados em nenhuma das 20 subscriptions. **Pendência de decisão humana:** o mesmo nome de RG existe em 2 subscriptions com conteúdos diferentes (`rg-oferta-data-prd`: 179 recursos em "PRD - ONLINE 2" e 50 em "PRD - ONLINE"; `rg-envvias-data-prd`: 1 e 6) — a app mostra o da subscription do cluster e AVISA do homônimo (`other_subscriptions`), sem somar.
- **Frontend:** `DataResourcesPanel` reescrito como tabela (tier, estado, custo, oferta, uso real ao expandir) e promovido a sub-aba própria **"Recursos de Dados"**, sempre disponível ao lado de "Discos Desatachados" (não depende de "Analisar").
- **Validado ao vivo** contra HLG e PRD reais e no navegador (Playwright + Chromium): `rg-oferta-hlg` — 5 VMs `D4s_v4` com CPU P95 ~2% / memória ~20% → oferta `D2s_v5` de R$ 788/mês cada; 9 VMs desalocadas com 16 discos ainda cobrando; até R$ 3.980/mês de economia potencial.

### FinOps — Discos Desatachados (Setembro 2026, branch `feat/vms-ec2-tab`, ainda não mesclada) ✅

Nova sub-aba **"Discos Desatachados"** no FinOps: lista Managed Disks (Azure), Persistent Disks (GCP) e volumes EBS (AWS) que existem na conta/projeto/subscription do cluster selecionado mas não estão atachados a nenhuma VM, com custo mensal e um veredito por disco. Endpoint `GET /api/v1/finops/unattached-disks?cluster=X[&refresh=true]`; só leitura — a app nunca exclui, cada disco traz o comando de exclusão pra copiar. Não depende de "Analisar" (a aba aparece assim que há um cluster).

- Descoberta por cloud em `internal/cloudprovider/{azure,gcp,aws}/unattached_disks.go`; preço/veredito/cruzamento com PVs em `internal/finops/unattached_disks.go`; handler em `internal/web/handlers/finops_unattached_disks.go` (cache de 5 min + singleflight, chaveado por subscription/projeto/região+profile — não por cluster).
- **Veredito** (`candidate` / `review` / `in_use_by_pv`): cruza cada disco com os PVs do cluster (`spec.csi.volumeHandle` e plugins in-tree). PV Bound aponta pro disco → `in_use_by_pv` (desatachado só porque nenhum pod o monta — workload escalado a 0). Sem cruzamento com o cluster (inacessível), **nada** é marcado candidato. Desatachado há < 7 dias rebaixa candidato pra `review`. Disco com marca de K8s mas cuja pista de cluster não bate com o selecionado é `review` (pode ser de outro cluster).
- **Achados reais (validados ao vivo contra Azure/GCP/AWS reais):** (1) `az disk list` **exige `--resource-group`** no CLI 2.88 — a descoberta Azure usa a ARM REST (`/subscriptions/{id}/providers/Microsoft.Compute/disks`, token do `az` já logado, paginação por `nextLink` restrita ao host `management.azure.com`); (2) a API devolve `LastOwnershipUpdateTime` com L maiúsculo (o Go casa sem diferenciar caixa); (3) em HLG os discos de PVC ficam desatachados à noite (clusters escalados a 0) — 35 de 35 discos de uma subscription eram assim, por isso o cruzamento com PVs é essencial; (4) `Reserved` (disco de VM desalocada) não entra: é outro desperdício; (5) uma subscription/projeto tem discos de VÁRIOS clusters — a UI filtra por "Deste cluster / De outros clusters / Sem vínculo com K8s".
- **Preço de tabela** (`internal/finops/disk_price_table.go`, `price_source: "table"`) pros tipos que as APIs por tier/GB não cobrem: Azure Ultra Disk e Premium SSD v2, GCP Hyperdisk (balanced, balanced-HA, extreme, ml, throughput) e EBS magnético (`standard`). Diferente das listas `*FallbackPrices` (só capacidade), aqui o custo soma capacidade + **IOPS e throughput provisionados** acima da cota gratuita (Premium SSD v2: 3.000 IOPS/125 MBps; Hyperdisk Balanced: 3.000 IOPS/140 MiB/s), lidos de `diskIOPSReadWrite`/`diskMBpsReadWrite` (ARM) e `provisionedIops`/`provisionedThroughput` (Compute). Valores capturados ao vivo em 2026-09-19 (Azure Retail brazilsouth, Cloud Billing São Paulo, AWS Price List us-east-1); a tabela vence as consultas por API (que mapeariam `PremiumV2_LRS` pra Premium SSD por prefixo). Nenhuma subscription acessível tinha Ultra/Premium v2 desatachado, então essas linhas foram validadas por teste com contas feitas à mão, não com disco real. Tipo desconhecido continua sem preço (`unsupported`).
- Limitações declaradas na própria UI: AWS varre só a região do cluster (EBS é regional; io1/io2/gp3 não incluem o custo de IOPS/throughput extra); o preço de tabela usa a região de referência, não a do disco; preços GCP por API usam a região do pricer.
